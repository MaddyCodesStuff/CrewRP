    ...
    shouldBuild(resourceName) {
        ...
        function shouldBuild(config) {
            ...
            for (const file of cache) {
                ...
                if (!stats || stats.mtime !== file.stats.mtime || stats.size !== file.stats.size || stats.inode !==
                    file.stats.inode) {
                    return true;
                }
            }
            ...
            for (const configName of configs) {
                ...
                if (config) {
                    ...
                    while (buildingInProgress) {
                        console.log(
                            `webpack is busy by another process: we are waiting to compile  ${resourceName} (${configName})`);
                        await sleep(3000);
                    }
