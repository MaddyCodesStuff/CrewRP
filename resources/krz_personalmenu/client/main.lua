...
local godmode, visible, gamerTags                                    = false, false, {}
...
AddEventHandler('esx_addonaccount:setMoney', function(society, money)
    if ESX and ESX.PlayerData and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.grade_name == 'boss' and 'society_' .. ESX.PlayerData.job.name == society then
        UpdateSocietyMoney(money)
    end
    if ESX and ESX.PlayerData and ESX.PlayerData.job2 ~= nil and ESX.PlayerData.job2.grade_name == 'boss' and 'society_' .. ESX.PlayerData.job2.name == society then
        UpdateSociety2Money(money)
    end
end)
...
function admin_tp_pos()
    ...
    if pos ~= nil and pos ~= '' then
        local a, b, x, y, z = string.find(pos, '([%d%.]+) ([%d%.]+) ([%d%.]+)')
...
function AddMenuInventoryMenu(menu)
    inventorymenu = _menuPool:AddSubMenu(menu, _U(''))
    --local invCount = {}
                            ...
                            if not IsPedSittingInAnyVehicle(closestPed) then
                                if quantity ~= nil and count > 0 then
                                    TriggerServerEvent('esx:giveInventoryItem',
                                                       GetPlayerServerId(personalmenu.closestPlayer), 'item_standard',
                                                       value, quantity)
                                    _menuPool:CloseAllMenus()
                                else
                    ...
                    weaponItemMenu.OnItemSelect = function(sender, item, index)
                        if item == giveItem then
                            ...
                            if foundPlayers == true then
                                ...
                                if not IsPedSittingInAnyVehicle(closestPed) then
                                    TriggerServerEvent('esx:giveInventoryItem',
                                                       GetPlayerServerId(personalmenu.closestPlayer), 'item_weapon',
                                                       value, ammo)
                                    _menuPool:CloseAllMenus()
                                else
                                    ESX.ShowNotification(_U('in_vehicle_give', label))
                                        ...
                                        if ammo > 0 then
                                            if post == true then
                                                if quantity <= ammo and quantity >= 0 then
                                                    ...
                                                    TriggerServerEvent('KorioZ-PersonalMenu:Weapon_addAmmoToPedS',
                                                                       GetPlayerServerId(personalmenu.closestPlayer),
                                                                       value, quantity)

                                                    ESX.ShowNotification(_U('gave_ammo', quantity,
                                                                            GetPlayerName(personalmenu.closestPlayer)))
                                                    _menuPool:CloseAllMenus()
                                                else
...
function AddMenuWalletMenu(menu)
    ...
    if Config.EnableJsfourIDCard then
        ...
        showHunting = NativeUI.CreateItem(_U('wallet_show_hunting_button'), '')
        walletmenu.SubMenu:AddItem(showHunting)

        checkHunting = NativeUI.CreateItem(_U('wallet_check_hunting_button'), '')
        walletmenu.SubMenu:AddItem(checkHunting)

        showPilot = NativeUI.CreateItem(_U('wallet_show_pilot_button'), '')
        walletmenu.SubMenu:AddItem(showPilot)

        checkPilot = NativeUI.CreateItem(_U('wallet_check_pilot_button'), '')
        walletmenu.SubMenu:AddItem(checkPilot)
    end
    ...
    walletmenu.SubMenu.OnItemSelect = function(sender, item, index)
        if Config.EnableJsfourIDCard then
            ...
            elseif item == showDriver then
                ...
                if personalmenu.closestDistance ~= -1 and personalmenu.closestDistance <= 3.0 then
                    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()),
                                       GetPlayerServerId(personalmenu.closestPlayer), 'drive')
                else
                    ESX.ShowNotification(_U('players_nearby'))
                end
            ...
            elseif item == showFirearms then
                ...
                if personalmenu.closestDistance ~= -1 and personalmenu.closestDistance <= 3.0 then
                    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()),
                                       GetPlayerServerId(personalmenu.closestPlayer), 'weapon')
                else
                    ESX.ShowNotification(_U('players_nearby'))
                end
            elseif item == checkFirearms then
                TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()),
                                   'weapon')
            elseif item == showHunting then
                personalmenu.closestPlayer, personalmenu.closestDistance = ESX.Game.GetClosestPlayer()

                if personalmenu.closestDistance ~= -1 and personalmenu.closestDistance <= 3.0 then
                    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()),
                                       GetPlayerServerId(personalmenu.closestPlayer), 'hunting')
                else
                    ESX.ShowNotification(_U('players_nearby'))
                end
            elseif item == checkHunting then
                TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()),
                                   'hunting')
            elseif item == showPilot then
                personalmenu.closestPlayer, personalmenu.closestDistance = ESX.Game.GetClosestPlayer()

                if personalmenu.closestDistance ~= -1 and personalmenu.closestDistance <= 3.0 then
                    TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()),
                                       GetPlayerServerId(personalmenu.closestPlayer), 'aircraft')
                else
                    ESX.ShowNotification(_U('players_nearby'))
                end
            elseif item == checkPilot then
                TriggerServerEvent('jsfour-idcard:open', GetPlayerServerId(PlayerId()), GetPlayerServerId(PlayerId()),
                                   'aircraft')
            end
        end
                    ...
                    if not IsPedSittingInAnyVehicle(closestPed) then
                        if post == true then
                            if item == walletMoney then
                                TriggerServerEvent('esx:giveInventoryItem',
                                                   GetPlayerServerId(personalmenu.closestPlayer), 'item_money', 'money',
                                                   quantity)
                                _menuPool:CloseAllMenus()
                            elseif item == walletdirtyMoney then
                                TriggerServerEvent('esx:giveInventoryItem',
                                                   GetPlayerServerId(personalmenu.closestPlayer), 'item_account',
                                                   'black_money', quantity)
                                _menuPool:CloseAllMenus()
                            end
                        else
                            ESX.ShowNotification(_U('amount_invalid'))
...
function AddMenuEmoteMenu(menu)
    emoteMenu        = _menuPool:AddSubMenu(menu, _U('emotes'))
    danceMenu        = _menuPool:AddSubMenu(emoteMenu.SubMenu, _U('danceemotes'))
    propMenu         = _menuPool:AddSubMenu(emoteMenu.SubMenu, _U('propemotes'))
    ...
    if Config.SharedEmotesEnabled then
        shareMenu       = _menuPool:AddSubMenu(emoteMenu.SubMenu, _U('shareemotes', _U('shareemotesinfo')))
        sharedDanceMenu = _menuPool:AddSubMenu(emoteMenu.SubMenu, _U('sharedanceemotes'))
    end
    ...
    for a, b in pairsByKeys(DP.Emotes) do
        ...
        emoteMenu.SubMenu:AddItem(emoteItem)
    end
        ...
        for a, b in pairsByKeys(DP.Shared) do
            ...
            if otherEmoteName == nil then
                ...
                shareItem = NativeUI.CreateItem(z,
                                                "/nearby ~g~" .. a .. "~w~ " .. _U('makenearby') .. " ~y~" .. otherEmoteName .. "~w~")
            end
            danceItem = NativeUI.CreateItem(z, "/e " .. a)
    ...
    for a, b in pairsByKeys(DP.PropEmotes) do
        x, y, z             = table.unpack(b)
        pushedPropEmotes[z] = a
        propItem            = NativeUI.CreateItem(z, "/e " .. a)
        propMenu.SubMenu:AddItem(propItem)
    end

    emoteMenu.SubMenu.OnItemSelect       = function(sender, item, index)
        EmoteMenuStart(pushedEmotes[item['Text']['_Text']], "emotes")
    end

    danceMenu.SubMenu.OnItemSelect       = function(sender, item, index)
        EmoteMenuStart(pushedDances[item['Text']['_Text']], "dances")
    end
    ...
    propMenu.SubMenu.OnItemSelect        = function(sender, item, index)
        EmoteMenuStart(pushedPropEmotes[item['Text']['_Text']], "props")
    end
end

function AddMenuCancelEmote(menu)
    cancelEmote = NativeUI.CreateItem(_U('cancelemote'), _U('cancelemoteinfo'))
    menu:AddItem(cancelEmote)
...
function AddMenuWalkMenu(menu)
    walkMenu  = _menuPool:AddSubMenu(menu, _U('walkingstyles'))
    walkReset = NativeUI.CreateItem(_U('normalreset'), _U('resetdef'))
    walkMenu.SubMenu:AddItem(walkReset)

    for a, _ in pairsByKeys(DP.Walks) do
...
function AddMenuFaceMenu(menu)
    faceMenu  = _menuPool:AddSubMenu(menu, _U('moods'))
    faceReset = NativeUI.CreateItem(_U('normalreset'), _U('resetdef'))
    faceMenu.SubMenu:AddItem(faceReset)

    for a, _ in pairsByKeys(DP.Expressions) do
        faceItem = NativeUI.CreateItem(a, '')
        faceMenu.SubMenu:AddItem(faceItem)
    end

    faceMenu.SubMenu.OnItemSelect = function(sender, item)
...
--I stole this from the emotes part of this resourse
-- i'll play around with IsPedMale later to find a more 
--elegant solution but for now I want to get the bug fixed
        ...
        if item == recruterItem then
            if ESX.PlayerData.job.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_recruterplayer',
                                       GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job.name, 0)
                end
            else
        ...
        elseif item == virerItem then
            if ESX.PlayerData.job.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_virerplayer',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
        ...
        elseif item == promouvoirItem then
            if ESX.PlayerData.job.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_promouvoirplayer',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
        ...
        elseif item == destituerItem then
            if ESX.PlayerData.job.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_destituerplayer',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
        ...
        if item == recruter2Item then
            if ESX.PlayerData.job2.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_recruterplayer2',
                                       GetPlayerServerId(personalmenu.closestPlayer), ESX.PlayerData.job2.name, 0)
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
            end
        elseif item == virer2Item then
            if ESX.PlayerData.job2.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_virerplayer2',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
        ...
        elseif item == promouvoir2Item then
            if ESX.PlayerData.job2.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_promouvoirplayer2',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
        ...
        elseif item == destituer2Item then
            if ESX.PlayerData.job2.grade_name == 'boss' then
                ...
                if personalmenu.closestPlayer == -1 or personalmenu.closestDistance > 3.0 then
                    ...
                    TriggerServerEvent('KorioZ-PersonalMenu:Boss_destituerplayer2',
                                       GetPlayerServerId(personalmenu.closestPlayer))
                end
            else
                ESX.ShowNotification(_U('missing_rights'))
...
function AddMenuDemarcheVoixGPS(menu)
    ...
    local demarcheItem = NativeUI.CreateListItem(_U('mainmenu_approach_button'), personalmenu.demarche,
                                                 actualDemarcheIndex)
...
function GeneratePersonalMenu(playerGroup)
    ...
    AddMenuEmoteMenu(mainMenu)
    AddMenuCancelEmote(mainMenu)

    if Config.WalkingStylesEnabled then
        AddMenuWalkMenu(mainMenu)
    end
        ...
        if showname then
            for k, v in ipairs(GetActivePlayers()) do
                ...
                if otherPed ~= plyPed then
                    if #(GetEntityCoords(plyPed) - GetEntityCoords(otherPed)) < 5000.0 then
