...
Citizen.CreateThread(function()
    while true do
        ...
        if IsControlJustPressed(1, Config.handsUP.clavier) and IsUsingKeyboard(2) then
            if (DoesEntityExist(plyPed)) and not (IsEntityDead(plyPed)) and (IsPedOnFoot(plyPed)) then
                ...
                Citizen.CreateThread(function()
                    ...
                    Wait(200)
                    if handsup then -- Double check button hasn't been released
        ...
        if handsup and IsControlReleased(1, Config.handsUP.clavier) and DoesEntityExist(plyPed)then
            handsup = false
            ClearPedSecondaryTask(plyPed)
        end

        if IsControlJustReleased(1, Config.pointing.clavier) and IsUsingKeyboard(2) then
            if (DoesEntityExist(plyPed)) and (not IsEntityDead(plyPed)) and (IsPedOnFoot(plyPed)) then
                if handsup then
                    handsup = false
                end
        ...
        if crouched or handsup or pointing then
            ...
            elseif pointing then
                ...
                local coords             = GetOffsetFromEntityInWorldCoords(ped,
                                                                            ...
                                                                            0.6)
                local rayHandle, blocked = GetShapeTestResult(StartShapeTestCapsule(coords.x, coords.y, coords.z - 0.2,
