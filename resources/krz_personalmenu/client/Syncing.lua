...
AddEventHandler("ClientEmoteRequestReceive", function(emotename, etype)
    ...
    if etype == 'Dances' then
        ...
        a, b, remote = table.unpack(DP.Shared[requestedemote])
    end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(5)
        if IsControlJustPressed(1, 168) and isRequestAnim then
            target, distance = GetClosestPlayer()
            if (distance ~= -1 and distance < 3) then
                if DP.Shared[requestedemote] ~= nil then
                    a, b, c, otheremote = table.unpack(DP.Shared[requestedemote])
                elseif DP.Dances[requestedemote] ~= nil then
                    a, b, c, otheremote = table.unpack(DP.Dances[requestedemote])
                end
...
function GetPlayerFromPed(ped)
    for a, player in ipairs(GetActivePlayers()) do
...
function GetPedInFront()
    ...
    local rayHandle        = StartShapeTestCapsule(plyPos.x, plyPos.y, plyPos.z, plyOffset.x, plyOffset.y, plyOffset.z,
                                                   10.0,
                                                   12, plyPed, 7)
    local a, b, c, d, ped2 = GetShapeTestResult(rayHandle)
    return ped2
end
