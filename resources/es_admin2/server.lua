...
TriggerEvent("es:addGroup", "mod", "user", function(group)
end)
...
function permBanUser(bannedBy, id)
    ...
    MySQL.Async.execute(
        "UPDATE users SET banned = 1 WHERE identifier = @identifier",
        {
            ['@identifier'] = id
        }
    )
        ...
        TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
            if available or user.getGroup() == "superadmin" then
                if type == "slay_all" then
                    TriggerClientEvent('es_admin:quick', -1, 'slay')
                end
                if type == "bring_all" then
                    TriggerClientEvent('es_admin:quick', -1, 'bring', Source)
                end
                if type == "slap_all" then
                    TriggerClientEvent('es_admin:quick', -1, 'slap')
                end
            else
                TriggerClientEvent('chat:addMessage', Source, {
            ...
            TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
                TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
                    if canTarget and available then
                        if type == "slay" then
                            TriggerClientEvent('es_admin:quick', id, type)
                        end
                        if type == "noclip" then
                            TriggerClientEvent('es_admin:quick', id, type)
                        end
                        ...
                        if type == "bring" then
                            TriggerClientEvent('es_admin:quick', id, type, Source)
                        end
                        if type == "goto" then
                            TriggerClientEvent('es_admin:quick', Source, type, id)
                        end
                        if type == "slap" then
                            TriggerClientEvent('es_admin:quick', id, type)
                        end
                        ...
                        if type == "kick" then
                            DropPlayer(id, 'Kicked by es_admin GUI')
                        end
                        ...
                        if type == "ban" then
                            ...
                            DropPlayer(id, "You have been banned. Consult Discord for more information.")
            ...
            if available then
                ...
                elseif t == "level" then
                    if (GetPlayerName(USER) == nil) then
                        ...
                        if (GROUP ~= nil and GROUP > -1) then
                            TriggerEvent("es:setPlayerData", USER, "permission_level", GROUP,
                ...
                elseif t == "ped" then
                    if (GetPlayerName(USER) == nil) then
                        ...
                        TriggerClientEvent('es_admin:setPed', USER, GROUP)
                    end
                elseif t == "resetped" then
                    if (GetPlayerName(USER) == nil) then
                        TriggerClientEvent('chat:addMessage', Source, {
                            args = { "^1SYSTEM", "Player not found" }
                        })
                    else
                        TriggerClientEvent('es_admin:resetPed', USER)
                    end
                end
            else
                TriggerClientEvent('chat:addMessage', Source, {
...
RegisterCommand('setadmin', function(source, args, raw)
    ...
    if args[1] then
        if (player and GetPlayerName(player)) then
            if level then
                TriggerEvent("es:setPlayerData", tonumber(args[1]), "permission_level", tonumber(args[2]),
                             function(response, success)
                                 RconPrint(response)

                                 TriggerClientEvent('es:setPlayerDecorator', tonumber(args[1]), 'rank',
                                                    tonumber(args[2]), true)
                                 TriggerClientEvent('chat:addMessage', -1, {
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
                    if canTarget then
                        ...
                        TriggerClientEvent('chat:addMessage', source,
                                           { args = { "^1SYSTEM", "You can not target this person" } })
                    end
                end)
            end)
        else
...
TriggerEvent('es:addGroupCommand', 'kick', "mod", function(source, args, user)
    ...
    TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "Insufficienct permissions!" } })
             ...
             { help = "Kick a user with the specified reason or no reason", params = { { name = "userid", help = "The ID of the player" }, { name = "reason", help = "The reason as to why you kick this player" } } })
...
TriggerEvent('es:addGroupCommand', 'announce', "admin", function(source, args, user)
    ...
    TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "Insufficienct permissions!" } })
             ...
             { help = "Announce a message to the entire server", params = { { name = "announcement", help = "The message to announce" } } })
    ...
    if args[1] then
        if (tonumber(args[1]) and GetPlayerName(tonumber(args[1]))) then
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                ...
                TriggerClientEvent('es_admin:teleportUser', target.get('source'), user.getCoords().x,
                                   user.getCoords().y, user.getCoords().z)
    ...
    if args[1] then
        if (tonumber(args[1]) and GetPlayerName(tonumber(args[1]))) then
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                ...
                TriggerClientEvent('chat:addMessage', player,
                                   { args = { "^1SYSTEM", "You have slapped by ^2" .. GetPlayerName(source) } })
                TriggerClientEvent('chat:addMessage', source,
                                   { args = { "^1SYSTEM", "Player ^2" .. GetPlayerName(player) .. "^0 has been slapped" } })
            end)
        else
            TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "Incorrect player ID" } })
        end
        ...
        if (tonumber(args[1]) and GetPlayerName(tonumber(args[1]))) then
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                if (target) then
                    ...
                    TriggerClientEvent('es_admin:teleportUser', source, target.getCoords().x, target.getCoords().y,
                                       target.getCoords().z)
...
TriggerEvent('es:addCommand', 'die', function(source, args, user)
    ...
    TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "You died" } })
end, { help = "Die" })
...
TriggerEvent('es:addGroupCommand', 'crash', "superadmin", function(source, args, user)
    if args[1] then
        if (tonumber(args[1]) and GetPlayerName(tonumber(args[1]))) then
            ...
            TriggerEvent("es:getPlayerFromId", player, function(target)
                ...
                TriggerClientEvent('chat:addMessage', source,
                                   { args = { "^1SYSTEM", "Player ^2" .. GetPlayerName(player) .. "^0 has been crashed." } })
            end)
        else
            TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "Incorrect player ID" } })
        end
    ...
    TriggerClientEvent('chat:addMessage', source, { args = { "^1SYSTEM", "Insufficienct permissions!" } })
             ...
             { help = "Crash a user, no idea why this still exists", params = { { name = "userid", help = "The ID of the player" } } })
...
ESX.RegisterServerCallback('es_admin:getSavedPed', function(source, cb)
...
RegisterServerEvent('es_admin:setSavedPed')
AddEventHandler('es_admin:setSavedPed', function(ped)
    ...
    MySQL.Async.execute(
        "UPDATE users SET ped = @ped WHERE identifier = @identifier",
        {
            ['@identifier'] = xPlayer.identifier,
            ['@ped']        = ped
        }
