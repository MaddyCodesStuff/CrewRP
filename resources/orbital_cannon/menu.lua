...
function Menu:canGoBack(b)
	self.cangoback = b
end

function Menu:canExit(b)
	self.canexit = b
end
...
local function table_contain(t,val)
...
local function KeyboardInput(TextEntry, ExampleText, MaxStringLenght)
	...
	inputactive = true --Blocks new input while typing if **inputactive** is used
	...
	if UpdateOnscreenKeyboard() ~= 2 then
		...
		inputactive = false --This unblocks new Input when typing is done
...
function Menu:ChangeMenu(menu,back)
	if self.OnMenuChange ~= nil then
		self:OnMenuChange(self.currentmenu,menu)
	end
	
	if back == nil or back ~= false  then
		local exist = false
		for i,m in pairs(self.lastmenu) do
			if m.name == menu.name then
				exist = true
			end
		end
		if not exist then
			table.insert(self.lastmenu,self.currentmenu)
		end
	end
	for i,m in pairs(self.lastmenu) do
		if m.name == menu.name then
			table.remove(self.lastmenu,i)
			break
		end
	end
	self.currentmenu = menu
	--Citizen.Trace("Last menu1: "..self.lastmenu[#self.lastmenu-1].name)
	--Citizen.Trace("Last menu2: "..self.lastmenu[#self.lastmenu].name)
	--Citizen.Trace("Current menu: "..self.currentmenu.name)
end
...
function Menu:addButton(name, info, args)
	local button = {}
	button.name = name 
...
function Menu:addRangeOfNumbers(name, minimum, maximum, info)
	local button = {}
	button.name = name 
	button.selected = false
	button.minimum = minimum
	button.maximum = maximum
	button.number = minimum
	button.sprite = nil
	button.info = info
	table.insert(self.buttons, button)
	return self.buttons[#self.buttons]
...
function Menu:addRangeOfFloats(name, minimum, maximum, step, info)
	local button = {}
	button.name = name 
	button.selected = false
	button.minimum = minimum
	button.maximum = maximum
	button.float = minimum
	button.step = step
	button.sprite = nil
	button.info = info
	table.insert(self.buttons, button)
...
function Menu:addInput(name, default, maxlength)
	local button = {}
	button.name = name 
	button.selected = false
	button.sprite = nil
	if default then
		...
		button.input = "~r~Not set"
	end
	if  maxlength then
		button.maxlength = maxlength
	else
		button.maxlength = 99999
	end
	table.insert(self.buttons, button)
	return self.buttons[#self.buttons]
end
...
function Menu:OnMenuChange(last,current)
	for i,m in pairs(self.lastmenu) do
...
function Menu:addSubMenu(header,name,info,addbtn)
	...
	if addbtn then
		table.insert(self.buttons, { name = addbtn, selected = false, menu = self[name], info = info })
	end
	...
	local lastselect = nil
		while self.visible do
			...
			ty = y - height/2.8
					...
					if i >= menu.from and i <= menu.to then
						if menu.selected == i then
							...
							elseif btn.menu ~= nil then
									DrawSprite("commonmenu", "arrowright", x + (width/2.2), y, scale/14 -0.01, scale/19 +0.01, 0.00,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
							end
							if IsPauseMenuActive() == false and inputactive == false and IsControlJustReleased(1,self.config.controls.menu_select) or IsDisabledControlJustReleased(1,self.config.controls.menu_select) then
								...
								if btn.input ~= nil then
									local txt = KeyboardInput(btn.name, btn.input, btn.maxlength)
									if txt ~= nil then
										btn.input = txt
									end
								end
							...
							if btn.checkbox ~= nil then
								if btn.checkbox then
									DrawSprite("commonmenu", "shop_box_tickb", x + (width/2.3), y, scale/12, scale/10, 0.00,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
								else
									DrawSprite("commonmenu", "shop_box_blankb",  x + (width/2.3), y, scale/12, scale/10, 0.00,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
								end
							elseif btn.list then
								...
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_left) or IsDisabledControlJustPressed(1,self.config.controls.menu_left)then
								...
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_right) or IsDisabledControlJustPressed(1,self.config.controls.menu_right) then
							...
							elseif btn.number then
								drawRightMenuText('< '..btn.number..'/'..btn.maximum..' >',cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_left) or IsDisabledControlJustPressed(1,self.config.controls.menu_left)then
									if btn.number > btn.minimum then
										btn.number = btn.number - 1
									else
										btn.number = btn.maximum
									end
									if self.OnButtonNumberChange ~= nil then
										self:OnButtonNumberChange(btn)
									end
								end
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_right) or IsDisabledControlJustPressed(1,self.config.controls.menu_right) then
									if btn.number < btn.maximum then
										btn.number = btn.number + 1
									else
										btn.number = btn.minimum
									end
									if self.OnButtonNumberChange ~= nil then
										self:OnButtonNumberChange(btn)
									end
								end
							elseif btn.float then
								drawRightMenuText('< '..btn.float..'/'..btn.maximum..' >',cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_left) or IsDisabledControlJustPressed(1,self.config.controls.menu_left)then
									if btn.float > btn.minimum then
										btn.float = btn.float - btn.step
									else
										btn.float = btn.maximum
									end
									if self.OnButtonNumberChange ~= nil then
										self:OnButtonNumberChange(btn)
									end
								...
								if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_right) or IsDisabledControlJustPressed(1,self.config.controls.menu_right) then
									if btn.float < btn.maximum then
										btn.float = btn.float + btn.step
									else
										btn.float = btn.minimum
									end
									if self.OnButtonNumberChange ~= nil then
										self:OnButtonNumberChange(btn)
									end
								end
							elseif btn.input then
								local str = btn.input
								if string.len(str) > 20 then
									str = string.sub(btn.input,1,20).."..."
								end
								drawRightMenuText(str,cfg.font,0,x + (width/2.1),ty,scale,cfg.stext_color.r,cfg.stext_color.g,cfg.stext_color.b,cfg.stext_color.a)
							end
						else
							...
							elseif btn.number then
								drawRightMenuText(btn.number..'/'..btn.maximum,cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
							elseif btn.float then
								drawRightMenuText(btn.float..'/'..btn.maximum,cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
							elseif btn.input then
								local str = btn.input
								if string.len(str) > 20 then
									str = string.sub(btn.input,1,20).."..."
								end
								drawRightMenuText(str,cfg.font,0,x + (width/2.1),ty,scale,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
							end
							...
							if btn.checkbox ~= nil then
								if btn.checkbox then
									DrawSprite("commonmenu", "shop_box_tick", x + (width/2.3), y, scale/12, scale/10, 0.00,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
								else
									DrawSprite("commonmenu", "shop_box_blank",  x + (width/2.3), y, scale/12, scale/10, 0.00,cfg.text_color.r,cfg.text_color.g,cfg.text_color.b,cfg.text_color.a)
								end
							end
						end
				...
				if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_up) or IsDisabledControlJustPressed(1,self.config.controls.menu_up)then
				...
				if IsPauseMenuActive() == false and inputactive == false and IsControlJustPressed(1,self.config.controls.menu_down) or IsDisabledControlJustPressed(1,self.config.controls.menu_down) then
				...
				if IsPauseMenuActive() == false and inputactive == false and IsControlJustReleased(1,  self.config.controls.menu_back) or IsDisabledControlJustReleased(1,  self.config.controls.menu_back) then
