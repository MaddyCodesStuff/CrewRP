...
AddEventHandler('mhacking:seqstart', function(solutionlength, duration, callback)
    ...
    elseif type(solutionlength) == 'table' and type(duration) == 'table' then
        ...
        local solTblLen = 0
        local durTblLen = 0
        for _ in ipairs(solutionlength) do
            solTblLen = solTblLen + 1
        end
        for _ in ipairs(duration) do
            durTblLen = durTblLen + 1
        end
        itrTbl = duration
        if solTblLen > durTblLen then
            itrTbl = solutionlength
        end
        TriggerEvent('mhacking:show')
