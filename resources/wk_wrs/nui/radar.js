...
function lockPlate(state) {
    if (state == true) {
        document.getElementById('fwdplate').style.background =
            'linear-gradient( to bottom, rgb( 220, 0, 40 ), rgb( 90, 0, 0 ) )';
        $('#fwdplate').addClass('locked')
    } else {
        document.getElementById('fwdplate').style.background =
            'linear-gradient( to bottom, rgb( 0, 150, 0 ), rgb( 0, 75, 0 ) )';
        $('#fwdplate').removeClass('locked')
    }
}
