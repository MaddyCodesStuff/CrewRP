...
function Menu.Option(option)
    ...
    elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
        GUI.Text(option, GUI.optionText,
                 { menuX - 0.1, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 },
                 false);
        GUI.Rect(GUI.optionRect, { menuX, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.1415 },
                 { 0.23, 0.035 });
        if (thisOption) then
            GUI.Rect(GUI.scroller, { menuX, optionCount - (currentOption - maxVisOptions) * 0.035 + 0.1415 },
                     { 0.23, 0.035 })
        end
    end
...
function Menu.Bool(option, bool, cb)
    ...
    elseif (optionCount > currentOption - GUI.maxVisOptions and optionCount <= currentOption) then
        if (bool) then
            GUI.Text("~g~ON", GUI.optionText,
                     { menuX + 0.068, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 },
                     true)
        else
            GUI.Text("~r~OFF", GUI.optionText,
                     { menuX + 0.068, optionCount - (currentOption - GUI.maxVisOptions) * 0.035 + 0.125 }, { 0.5, 0.5 },
                     true)
        end
    end
...
function Menu.updateSelection()
    ...
    if IsDisabledControlJustPressed(1, 173) then
        --Down
        if (currentOption < optionCount) then
    ...
    elseif IsDisabledControlJustPressed(1, 172) then
        --Up
    ...
    elseif IsDisabledControlJustPressed(1, 174) then
        --Left
        leftPressed = true
    elseif IsDisabledControlJustPressed(1, 175) then
        --Right
        rightPressed = true
    elseif IsDisabledControlJustPressed(1, 176) then
        --Select
        selectPressed = true
    end
    optionCount = 0
