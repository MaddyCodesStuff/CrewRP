...
AddEventHandler('esx_vangelico_robbery:toofar', function(robb)
    ...
    if (robbers[source]) then
        ...
        TriggerClientEvent('mythic_notify:client:SendErrorAlert', source,
                           { text = _U('robbery_has_cancelled') .. Stores[robb].nameofstore })
    end
end)
...
AddEventHandler('esx_vangelico_robbery:endrob', function(robb)
    ...
    if (robbers[source]) then
        ...
        TriggerClientEvent('mythic_notify:client:SendSuccessAlert', source,
                           { text = _U('robbery_has_ended') .. Stores[robb].nameofstore })
    end
end)
...
AddEventHandler('esx_vangelico_robbery:rob', function(robb)
    ...
    if Stores[robb] then
        ...
        if (os.time() - store.lastrobbed) < Config.SecBetwNextRob and store.lastrobbed ~= 0 then
            ...
            TriggerClientEvent('mythic_notify:client:SendErrorAlert', source,
                               { text = _U('already_robbed') .. (Config.SecBetwNextRob - (os.time() - store.lastrobbed)) .. _U('seconds') })
            return
        end
        ...
        if rob == false then
            ...
            for i = 1, #xPlayers, 1 do
                ...
                if xPlayer.job.name == 'police' or xPlayer.job.name == 'journalist' then
                    TriggerClientEvent('mythic_notify:client:SendCopAlert', xPlayers[i],
                                       { text = 'A jewelry robbery is in progress at <em>' .. store.nameofstore .. '</em>!', title = 'Jewelry Heist Reported' })
            ...
            TriggerClientEvent('mythic_notify:client:SendSuccessAlert', source,
                               { text = _U('started_to_rob') .. store.nameofstore .. _U('do_not_move') })
