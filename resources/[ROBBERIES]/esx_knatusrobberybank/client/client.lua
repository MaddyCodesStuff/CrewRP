...
AddEventHandler('esx_holdupbank:currentlyrobbing', function(robb)
    holdingup        = true
    bank             = robb
    secondsRemaining = math.floor(Banks[bank].timetotorch / 1000)
end)
...
AddEventHandler('esx_holdupbank:hackcomplete', function()
    hackholdingup = false
    TriggerEvent('mythic_notify:client:SendSuccessAlert', { text = _U('hack_complete') })

    TriggerServerEvent('esx_holdupbank:opendoor', Banks[bank])
...
AddEventHandler('esx_holdupbank:plantbombcomplete', function(bank)
    ...
    TriggerEvent('mythic_notify:client:SendSuccessAlert', { text = _U('bombplanted_run') })
    TriggerServerEvent('esx_holdupbank:plantbombtoall', bank.bombposition.x, bank.bombposition.y, bank.bombposition.z,
                       bank.bombdoortype)
...
AddEventHandler('esx_holdupbank:opendoors', function(bank)
    dooropen       = true;

    local coords   = { bank.position["x"], bank.position["y"], bank.position["z"] }
    local doortype = bank.doortype
    local rotation = GetEntityHeading(obs) + bank.yaw
    globalcoords   = coords
...
Citizen.CreateThread(function()
    while true do
        ...
        if holdingup then
            ...
            drawTxt(0.66, 1.44, 1.0, 1.0, 0.4, _U('robbery_of') .. secondsRemaining .. _U('seconds_remaining'), 255,
                    255, 255, 255)
            ...
            if pos2 then
                if (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0) then
                    if not hackholdingup then
                        DrawMarker(1, v.hackposition.x, v.hackposition.y, v.hackposition.z - 1, 0, 0, 0, 0, 0, 0,
                                   1.0001, 1.0001, 1.5001, 1555, 0, 0, 255, 0, 0, 0, 0)
...
Citizen.CreateThread(function()
    while true do
            ...
            if (pos2 ~= nil) then
                if (Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 15.0) then
                    if not bombholdingup then
                        DrawMarker(1, v.bombposition.x, v.bombposition.y, v.bombposition.z - 1, 0, 0, 0, 0, 0, 0,
                                   1.0001, 1.0001, 1.5001, 1555, 0, 0, 255, 0, 0, 0, 0)
        ...
        if bombholdingup then
            ...
            drawTxt(0.66, 1.44, 1.0, 1.0, 0.4, _U('bomb_of') .. secondsRemaining .. _U('seconds_remaining'), 255, 255,
                    255, 255)
            DisplayHelpText(_U('press_to_cancel'))
