...
-- Script starts here
local helicam          = true
local pd-mav_hash    = GetHashKey("pd-mav")
local fov              = (fov_max + fov_min) * 0.5
local vision_state     = 0 -- 0 is normal, 1 is nightmode, 2 is thermal vision
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsPlayerInpd-mav() then
                    ...
                    if DoesEntityExist(locked_on_vehicle) then
                        ...
                        if IsControlJustPressed(0, toggle_lock_on) then
                            ...
                            local rot         = GetCamRot(cam,
                                                          2) -- All this because I can't seem to get the camera unlocked from the entity
...
-- disabled spotlight as not needed for FD
...
function IsPlayerInpd-mav()
    local lPed    = GetPlayerPed(-1)
    local vehicle = GetVehiclePedIsIn(lPed)
    return IsVehicleModel(vehicle, pd-mav_hash)
end
...
function CheckInputRotation(cam, zoomvalue)
    ...
    if rightAxisX ~= 0.0 or rightAxisY ~= 0.0 then
        ...
        new_x = math.max(math.min(20.0, rotation.x + rightAxisY * -1.0 * (speed_lr) * (zoomvalue + 0.1)),
                         -89.5) -- Clamping at top (cant see top of heli) and at bottom (doesn't glitch out in -90deg)
