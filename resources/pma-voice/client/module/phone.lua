...
RegisterNetEvent('pma-voice:removePlayerFromCall', function(plySource)
	if plySource == playerServerId then
		...
		MumbleClearVoiceTargetPlayers(1)
		playerTargets(radioPressed and radioData or {}, callData)
		plyState:set('callChannel', 0, GetConvarInt('voice_syncData', 1) == 1)
	else
...
function setCallChannel(channel)
	...
	callChannel = channel
	plyState:set('callChannel', channel, GetConvarInt('voice_syncData', 1) == 1)
...
RegisterNetEvent('pma-voice:clSetPlayerCall', function(_callChannel)
	...
	plyState:set('callChannel', _callChannel, GetConvarInt('voice_syncData', 1) == 1)
	createPhoneThread()
end)
