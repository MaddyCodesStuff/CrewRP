...
local damageFactorPetrolTank      = 70.0                -- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 64
...
local degradingHealthSpeedFactor  = 5            -- Speed of slowly degrading health, but not failure. Value of 10 means that it will take about 0.25 second per health point, so degradation from 800 to 305 will take about 2 minutes of clean driving.
local degradingFailureThreshold   = 300.0            -- Below this value, slow health degradation will set in
local cascadingFailureThreshold   = 100.0            -- Below this value, health cascading failure will set in
local engineSafeGuard             = 75.0                    -- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
