...
AddEventHandler('chatMessage', function(from, name, message)
    if (string.sub(message, 1, 1) == "/") then
        ...
        if (cmd == "/taxifare") then
            ...
            if (subCmd == nil) then
                TriggerClientEvent('chatMessage', from, "Taxi Meter -- Settings", { 200, 0, 0 },
                                   "Usage: /taxifare help")
                return
            end
            if subCmd == "help" then
                TriggerClientEvent('chatMessage', from, "Taxi Meter -- Help", { 200, 0, 0 },
                                   "Possible actions: initial, mile, minute, show. Initial is the base starting fare (default: $50). Mile is the rate per mile driven (default: $35). Minute is the rate per minute while the meter is on (default: $100). Show will display your current settings.")
                return
            end
            if subCmd == "show" then
