...
local fareCost      = 0.20 --(1.66 = $100 per minute) Cost per second
local costPerMile   = 2.0
        ...
        while true do
            ...
            if (IsInTaxi()) then
                if (inTaxi == false) then
                    msg = "Press Ctrl + B to toggle Taxi Meter"
                    TriggerEvent("pNotify:SendNotification",
                                 { text = msg, type = "success", layout = "centerLeft", queue = "global", theme = "gta", timeout = 5000 })
                end
                inTaxi    = true
                local ped = GetPlayerPed(-1)
                local veh = GetVehiclePedIsIn(ped, false)
...
function IsNearPlayer(player)
    ...
    local distance   = GetDistanceBetweenCoords(ply2Coords["x"], ply2Coords["y"], ply2Coords["z"], plyCoords["x"],
