    ...
    switch (event.data.action) {
        case 'tick':
            ...
            if (event.data.street2 !== '') {
                $('.position').html(event.data.direction + ' <span class="seperator">|</span> ' + event.data.street1 +
                                    ' <span class="seperator2">-</span> ' + event.data.street2 +
                                    ' <span class="seperator">|</span> ' + event.data.area);
            } else {
                $('.position').html(event.data.direction + ' <span class="seperator">|</span> ' + event.data.street1 +
                                    ' <span class="seperator">|</span> ' + event.data.area);
            }
        ...
        case 'updateStatus':
            ...
            if (event.data.drunk === 0) {
                ...
                $('#drunk').css('opacity', '0.5');
            }
            $('#boxHigh').css('width', event.data.high + '%');
            if (event.data.high === 0) {
                $('#high').css('opacity', '0');
            } else {
                $('#high').css('opacity', '0.5');
            }
            $('#boxPoison').css('width', event.data.poison + '%');
            if (event.data.poison === 0) {
                $('#poison').css('opacity', '0');
            } else {
                $('#poison').css('opacity', '0.5');
            }
            break;
        case 'hidecar':
            $('.car').fadeOut();
