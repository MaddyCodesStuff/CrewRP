...
function SetGridTargets(pos, reset) -- Used to set the players voice targets depending on where they are in the map
	...
	if reset then
		MumbleSetAudioInputDistance(mumbleConfig.voiceModes[playerData.mode][1] + 0.0) -- Set voice proximity
		MumbleClearVoiceTarget(voiceTarget) -- Reset voice target
		Citizen.CreateThread(function ()
			while not HUDStarted() do
				Wait(100)
			end
			TriggerEvent('tcrphud:setVoiceLevel', playerData.mode)
		end)
	end
...
function TogglePlayerVoice(serverId, value, voiceType)
	local msg = false
    local volume = mumbleConfig.speakingVolume
    local submixId = -1

    if voiceType == 'radio' then
        volume = mumbleConfig.radioVolume
        submixId = radioSubmixId
    elseif voiceType == 'call' then
        volume = mumbleConfig.callVolume
        submixId = callSubmixId
    end
    DebugMsg("Player "..serverId.." submix "..submixId)
...
function SetRadioChannel(channel)
	...
	if channel ~= nil then
		...
		TriggerEvent('tcrphud:setRadioVolume', mumbleConfig.radioVolume)
					...
					if not unmutedPlayers[id] then
						...
						if playerData ~= nil then
							if playerData.radioActive then
								TogglePlayerVoice(id, true, 'radio')
							end
						end
					end
				end
	...
	if channel ~= nil then
		...
		if callData[channel] then -- Unmute current call participants
			for id, _ in pairs(callData[channel]) do
				if id ~= playerServerId then
					if not unmutedPlayers[id] then
						TogglePlayerVoice(id, true, 'call')
					end
				end
			end
		end
...
function createSubmixes()
    -- Create radio Submix (FX)
    radioSubmixId = CreateAudioSubmix('Radio')
    SetAudioSubmixEffectRadioFx(radioSubmixId, 0)
    SetAudioSubmixEffectParamInt(radioSubmixId, 0, GetHashKey('default'), 1)
    AddAudioSubmixOutput(radioSubmixId, 0)
    DebugMsg('Created submix '..radioSubmixId..' for radio')

    -- Create call Submix (FX)
    callSubmixId = CreateAudioSubmix('Call')
    SetAudioSubmixEffectRadioFx(callSubmixId, 1)
    SetAudioSubmixEffectParamInt(callSubmixId, 1, GetHashKey('default'), 1)
    SetAudioSubmixEffectParamFloat(callSubmixId, 1, GetHashKey('freq_low'), 700.0)
    SetAudioSubmixEffectParamFloat(callSubmixId, 1, GetHashKey('freq_hi'), 15000.0)
    AddAudioSubmixOutput(callSubmixId, 1)
    DebugMsg('Created submix '..callSubmixId..' for calls')
end

function isPlayerTalking()
    return IsControlPressed(0, 249) and true or false
end

function HUDStarted()
    return (GetResourceState('tcrp_hud') == "started")
end
...
AddEventHandler("onClientResourceStart", function(resName) -- Initialises the script, sets up voice range, voice targets and request sync with server
    ...
    MumbleSetAudioOutputDistance(mumbleConfig.voiceModes[3][1] + 0.0)
	...
	elseif key == "call" and callChannel ~= value then
		...
		if value > 0 then
			...
			if CompareChannels(playerData, "call", value) then
				if playerServerId ~= player then
					TogglePlayerVoice(player, value, 'call')
	...
	elseif key == "radioActive" and radioActive ~= value then
		...
		if radioChannel > 0 then
			if CompareChannels(playerData, "radio", radioChannel) then -- Check if player is in the same radio channel as you
				if playerServerId ~= player then
					if value then
						TogglePlayerVoice(player, true, 'radio') -- unmute player
					else
						if not vehicleTargets[player] then -- Mute if player is not in client vehicle
							if playerData.call > 0 then -- Check if the client is in a call
								if not CompareChannels(voiceData[player], "call", playerData.call) then -- Check if the client is in a call with the unmuted player
				...
				if voiceData[player][key] ~= nil then
					if voiceData[player][key][id] then
						...
						if playerServerId == id then -- Check if the client is gonna hear a nearby call
							TogglePlayerVoice(player, true, 'call') -- Unmute
						end
...
-- Set radio volume
RegisterNetEvent("mumble:setRadioVolume")
AddEventHandler("mumble:setRadioVolume", function(volume)
	mumbleConfig.radioVolume = volume
	if voiceData and voiceData[playerServerId] and voiceData[playerServerId].radio > 0 then
		local radioChannel = voiceData[playerServerId].radio
		SetRadioChannel(0)
		Wait(100)
		...
		if initialised then
			...
			if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
				...
				if not secondaryPressed then
					...
					MumbleSetAudioInputDistance(mumbleConfig.voiceModes[voiceMode][1] + 0.0)
					if HUDStarted() then
						TriggerEvent('tcrphud:setVoiceLevel', voiceMode)
					end
				...
				if not mumbleConfig.controls.radio.pressed then
					if IsControlJustPressed(0, mumbleConfig.controls.radio.key) and IsInputDisabled(0) then
						if playerData.radio > 0 then
							...
							if HUDStarted() then
								TriggerEvent('tcrphud:setRadioTalking', true)
							end
							...
							Citizen.CreateThread(function()
								...
								if HUDStarted() then
									TriggerEvent('tcrphud:setRadioTalking', false)
								end
...
-- Track player talking status
Citizen.CreateThread(function()
	while not initialised do
		Wait(10)
	end

	local isTalking = isPlayerTalking()
	while true do
		local _isTalking = isPlayerTalking()
		if _isTalking ~= isTalking then
			isTalking = _isTalking
			TriggerEvent('tcrphud:setTalking', isTalking)
		end
		Wait(10)
	end
end)
			...
			if mumbleConfig.use2dAudioInVehicles then
				...
				if IsPedInAnyVehicle(playerPed, false) then
						...
						for i = 1, #playerList do
							...
							if playerId ~= remotePlayerId then
								...
								if passengers[remotePlayerPed] then
									local remotePlayerServerId = GetPlayerServerId(remotePlayerId)
									MumbleSetVolumeOverrideByServerId(remotePlayerServerId, mumbleConfig.volume2dAudioInVehicles)
					...
					if wasPlayerInVehicle then
						...
						wasPlayerInVehicle = false
						SetPlayerTargets(callTargets, speakerTargets, vehicleTargets, playerData.radioActive and radioTargets or nil)
						MumbleSetVolumeOverride(playerPed, -1)
					end
				end
			end
...
exports("getRadioChannel", function()
    if voiceData and voiceData[playerServerId] then
        local playerData = voiceData[playerServerId]
        return playerData.radio
    end
    return 0
end)
...
exports("getPlayerVoiceMode", function()
    if voiceData and voiceData[playerServerId] then
        local playerData = voiceData[playerServerId]
        return playerData.mode
    end
    return 0
end)

exports("getPlayerTalking", isPlayerTalking)
exports("getPlayerTalkingRadio", function()
    if voiceData and voiceData[playerServerId] then
        local playerData = voiceData[playerServerId]
        return playerData.radioActive
    end
    return false
end)