...
app.get('getmyip', (req, res) => {
    const ip = (lodash.get(req, `headers.['x-forwarded-for']`) || lodash.get(req, `headers.['x-real-ip']`) ||
                lodash.get(req, 'connection.remoteAddress')).replace('::ffff:', '');
    res.send(ip);
});

http.on('upgrade', (req, socket) => {
    if (!req._query || !req._query.from) {
        return socket.destroy();
    }
    if (req._query.from === 'ts3' && !req._query.uuid) {
        return socket.destroy();
    }
});
...
io.on('connection', async socket => {
    ...
    socket.clientIp =
        socket.handshake.headers['x-forwarded-for'] || socket.request.connection.remoteAddress.replace('::ffff:', '');
    socket.safeIp   = Buffer.from(socket.clientIp).toString('base64');
    if (socket.clientIp.includes('::1') || socket.clientIp.includes('127.0.0.1') ||
    ...
    log('log', chalk`{${socket.from === 'ts3' ? 'cyan' :
        'yellow'} ${socket.from}} | Connection {cyan opened} - ${socket.safeIp}`);
    ...
    if (socket.from === 'ts3') {
        ...
        log('log', chalk`{${socket.from === 'ts3' ? 'cyan' :
            'yellow'} ${socket.from}} | Handshake {green successful} - ${socket.safeIp}`);

        socket.on('setTS3Data', data => setTS3Data(socket, data));
        socket.on('onTalkStatusChanged', data => setTS3Data(socket, {
            key:   'talking',
            value: data
        }));
        ...
        socket.on('updateClientIP', data => {
            if (!data || !data.ip || !IPv4Regex.test(data.ip) || !clients[socket.uuid]) {
                return;
            }
...
function onIncomingData(socket, data) {
    const client = clients[socket.uuid];
    if (!socket.uuid || !client || !client.ts3.socket) {
        return;
    }
...
async function onSocketDisconnect(socket) {
    log('log', chalk`{${socket.from === 'ts3' ? 'cyan' :
        'yellow'} ${socket.from}} | Connection {red lost} - ${socket.safeIp}`);
    ...
    if (socket.uuid && clients[socket.uuid]) {
        ...
        if (client[secondary].socket) {
            ...
            if (secondary === 'ts3') {
                ...
                registerHandshake(client[secondary].socket);
            }
        }
    }
}
...
function socketHeartbeat(socket) {
    ...
    socket.once('pong', _ => {
        ...
        clients[socket.uuid].latency = lodash.get(clients[socket.uuid], 'fivem.socket.latency', 0) +
