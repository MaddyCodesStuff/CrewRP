...
local function updatePositions()
    ...
    if selfPosition ~= lastPosition then
        TriggerEvent('chatMessage', '', { 0, 0, 0 },
                     'position changed to ' .. tostring(selfPosition) .. ' from ' .. tostring(lastPosition))
    end
...
AddEventHandler('race:hurryUp', function(time)
    CreateThread(function()
        ...
        while not resultsShown and not weFinished do
            ...
            echo(tostring(math.floor(time / 1000)):lpad(2,
                                                        '0') .. ':' .. tostring(math.floor((time % 1000) / 100)):lpad(2,
                                                                                                                      '0') .. "\n")
        end
    end)
end)
...
local function setUpCheckpoint(cp, next)
    ...
    cp.handle = CreateCheckpoint(typeNum, cp.pos[1], cp.pos[2], cp.pos[3] + 2.5, nextPos[1], nextPos[2], nextPos[3],
                                 1.0001, _r)
