	...
	MySQL.Async.fetchAll('SELECT * FROM weashops', {}, function(result)
		for i = 1, #result, 1 do
			...
			table.insert(shopItems[result[i].zone], {
				...
				licenseRequired = result[i].license_required,
				ammo			= result[i].ammo,
			})
		end
...
ESX.RegisterServerCallback('esx_weaponshop:buyWeapon', function(source, cb, weaponName, zone, ammo)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.hasWeapon(weaponName) then
		TriggerClientEvent('mythic_notify:client:SendErrorAlert', source, { text = _U('already_owned') })
		cb(false)
	else
		if zone == 'BlackWeashop' then
			...
			if xPlayer.getMoney() >= price then
				...
				TriggerClientEvent('mythic_notify:client:SendErrorAlert', source, { text = _U('not_enough') })
				cb(false)
			end
