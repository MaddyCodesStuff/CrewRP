TokoVoipConfig = {
    ...
    keySwitchChannels          = Keys["F1"], -- Keybind used to switch the radio channels
    ...
    wsServer                   = GetConvar('ws_server_ip', "147.135.104.6:33250"), -- Address of the websocket server
    ...
    plugin_data                = {
        ...
        TSChannel          = GetConvar('tokovoip_ts_channel', "GTAMain"),
        TSPassword         = "Banana", -- TeamSpeak channel password (can be empty)
        ...
        TSChannelWait      = "TokoVoIP Lobby", -- You NEED tokovoip in the wait channel name!

        -- Blocking screen informations
        TSServer           = GetConvar('tokovoip_ts_server_ip', "ts.thecrewrp.net"), -- TeamSpeak server address to be displayed on blocking screen
        TSChannelSupport   = "TokoVoIP Lobby", -- TeamSpeak support channel name displayed on blocking screen
...
AddEventHandler("onClientResourceStart", function(resource)
    if (resource == GetCurrentResourceName()) then
        --	Initialize the script when this resource is started
