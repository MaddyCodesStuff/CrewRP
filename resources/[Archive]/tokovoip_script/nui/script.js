    ...
    if (voipStatus !== OK) {
        ...
        if (res) {
            ...
            if (websocket && websocket.readyState === websocket.OPEN) {
                websocket.send(`42${JSON.stringify(['updateClientIP', {ip: clientIp}])}`);
            }
        }
    }
...
async function init(address, serverId) {
    ...
    websocket.onmessage = (evt) => {
        ...
        if (msg.event === 'setTS3Data') {
            ...
            if (msg.data.talking !== undefined) {
                $.post(`http://${scriptName}/setPlayerTalking`, JSON.stringify({state: (msg.data.talking) ? 1 : 0}));
            }
        }
        ...
        if (msg.event === 'onLatency') {
            latency                                      = msg.data;
            document.querySelector('#latency').innerHTML =
                `Latency Total: ${latency.total}ms<br>Latency FiveM: ${latency.fivem}ms<br>Latency TS3: ${latency.ts3}ms`;
        }
    };
    ...
    websocket.onclose = () => {
        ...
        } else if (event.code == 1010) // Note that this status code is not used by the server, because it can fail the WebSocket
                        // handshake instead.
        {
            reason =
...
function checkPluginVersion() {
    if (isPluginVersionCorrect()) {
        document.getElementById('pluginVersion').innerHTML =
            `Plugin version: <font color="green">${voip.pluginVersion}</font> (up-to-date)`;
    } else {
        document.getElementById('pluginVersion').innerHTML =
            `Plugin version: <font color="red">${voip.pluginVersion}</font> (Required: ${voip.minVersion})`;
        voipStatus                                         = INCORRECT_VERSION;
    }
}

function isPluginVersionCorrect() {
    if (!voip.pluginVersion) {
        return false;
    }
    if (parseInt(voip.pluginVersion.replace(/\./g, '')) < parseInt(voip.minVersion.replace(/\./g, ''))) {
        return false;
    }
    return true;
}
...
function updateTokovoipInfo(msg) {
    ...
    switch (voipStatus) {
        ...
        case WRONG_SERVER:
            msg           =
                `Connected to the wrong TeamSpeak server, please join the server: <font color="#01b0f0">${voip.plugin_data.TSServer}</font>`;
            screenMessage = 'Wrong TeamSpeak server';
            color         = 'red';
            break;
        case WRONG_CHANNEL:
            msg           =
                `Connected to the wrong TeamSpeak channel, please join the channel: <font color="#01b0f0">${voip.plugin_data.TSChannelWait &&
                                                                                                            voip.plugin_data.TSChannelWait !==
                                                                                                            '' &&
                                                                                                            voip.plugin_data.TSChannelWait ||
                                                                                                            voip.plugin_data.TSChannel}</font>`;
    ...
    document.getElementById('pluginStatus').innerHTML =
        `Plugin status: <font color="${color}">${screenMessage || msg}</font>`;
}
...
function updateConfig(payload) {
    voip                                               = payload;
    document.getElementById('TSServer').innerHTML      =
        `TeamSpeak server: <font color="#01b0f0">${voip.plugin_data.TSServer}</font>`;
    document.getElementById('TSChannel').innerHTML     =
        `TeamSpeak channel: <font color="#01b0f0">${(voip.plugin_data.TSChannelWait) ?
            voip.plugin_data.TSChannelWait.replace(/\[[a-z]spacer(.*?)\]/, '') :
            voip.plugin_data.TSChannel.replace(/\[[a-z]spacer(.*?)\]/, '')}</font>`;
    document.getElementById('TSDownload').innerHTML    = voip.plugin_data.TSDownload;
    document.getElementById('pluginVersion').innerHTML =
        `Plugin version: <font color="red">Not found</font> (Minimal version: ${voip.minVersion})`;
}
