...
function ProgressWithStartEvent(action, start, finish)
    ...
    if not IsEntityDead(GetPlayerPed(-1)) or mythic_action.useWhileDead then
        if not isDoingAction then
            ...
            TriggerEvent("mythic_base:client:SendAlert",
                         { text = "Already Doing An Action", type = "error", layout = "topRight", timeout = 1500 })
        end
    else
        TriggerEvent("mythic_base:client:SendAlert",
                     { text = "Cannot Perform An Action While Dead", type = "error", layout = "topRight", timeout = 1500 })
    end
end
    ...
    while true do
        if isDoingAction then
                ...
                if mythic_action.animation ~= nil then
                    ...
                    elseif mythic_action.animation.animDict ~= nil and mythic_action.animation.anim ~= nil then
                        ...
                        if (DoesEntityExist(player) and not IsEntityDead(player)) then
                            ...
                            TaskPlayAnim(player, mythic_action.animation.animDict, mythic_action.animation.anim, 3.0,
                                         1.0, -1, mythic_action.animation.flags, 0, 0, 0, 0)
                        end
                    else
            ...
            if not isProp and mythic_action.prop ~= nil and mythic_action.prop.model ~= nil then
                ...
                local modelSpawn = CreateObject(GetHashKey(mythic_action.prop.model), pCoords.x, pCoords.y, pCoords.z,
                                                true, true, true)
                ...
                AttachEntityToEntity(modelSpawn, GetPlayerPed(PlayerId()),
                                     GetPedBoneIndex(GetPlayerPed(PlayerId()), mythic_action.prop.bone),
                                     mythic_action.prop.coords.x, mythic_action.prop.coords.y,
                                     mythic_action.prop.coords.z, mythic_action.prop.rotation.x,
                                     mythic_action.prop.rotation.y, mythic_action.prop.rotation.z, 1, 1, 0, 1, 0, 1)
                prop_net = netid
