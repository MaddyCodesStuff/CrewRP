...
AddEventHandler('es:playerLoaded', function(source)
    SetTimeout(20000, function()
        ...
        MySQL.Async.fetchAll('SELECT TIMESTAMPDIFF(SECOND, CURRENT_TIMESTAMP,J_Time) as timeleft, J_Cell as cellule, isjailed as isjailed from jail where identifier =@id',
                             { ['@id'] = identifier }, function(result)
                if result[1] ~= nil then
                    if result[1].timeleft > 0 then
                        if result[1].cellule ~= nil then
                            TriggerClientEvent("esx_jb_jailer:JailInStation", source, result[1].cellule,
                                               result[1].timeleft)
                        end
...
AddEventHandler("esx_jb_jailer:PutInJail", function(playerid, jail, jailtime)
    ...
    if jail ~= nil then
        ...
        TriggerEvent('esx:importantlogs',
                     "[esx_jb_jailer] Mettre en prison en " .. jail .. " :" .. GetPlayerName(playerid) .. " pour " .. jailtime .. " secs - commande entree par " .. GetPlayerName(source))
        MySQL.Async.execute("INSERT INTO jail (identifier,isjailed,J_Time,J_Cell,Jailer,Jailer_ID) VALUES (@Identifier,@isjailed,@J_Time,@J_Cell,@JAILER,@JID) ON DUPLICATE KEY UPDATE identifier=@identifier,isjailed=@isjailed,J_Time=@J_Time, J_Cell=@J_Cell, Jailer=@JAILER, Jailer_ID=@JID",
                            { ['@identifier'] = identifier, ['@isjailed'] = true, ['@J_Time'] = year .. '-' .. month .. '-' .. day .. ' ' .. hour .. ':' .. minutes .. ':' .. seconds, ['@J_Cell'] = jail, ['@JAILER'] = name, ['@JID'] = id })
