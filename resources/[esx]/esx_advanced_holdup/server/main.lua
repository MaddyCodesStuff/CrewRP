...
AddEventHandler('esx_advanced_holdup:robberyInProgress', function(mainZone)
	...
	if Config.Zones[mainZone] then
		...
		if os.time() > (lastRobbed + Config.TimeBetweenRobs * 60) then
			...
			for i = 1, #xPlayers, 1 do
				local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				local blip = {["x"] = Config.Zones[mainZone].Pos.x, ["y"] = Config.Zones[mainZone].Pos.y, ["z"] = Config.Zones[mainZone].Pos.z, ["longrange"] = false, ["text"] = "[PD] " .. mainZone .. " Robbery", ["sprite"] = 161, ["color"] = 1, ["scale"] = 2.0, ["duration"] = Config.Zones[mainZone].TimeToRob}
