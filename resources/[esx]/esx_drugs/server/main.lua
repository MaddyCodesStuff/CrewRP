...
AddEventHandler('esx_drugs:sellDrug', function(itemName, amount)
	...
	TriggerClientEvent('esx:showNotification', source,
					   _U('dealer_sold', amount, xItem.label, ESX.Math.GroupDigits(price)))
end)
...
RegisterServerEvent('esx_drugs:pickedUpCocaLeafs')
AddEventHandler('esx_drugs:pickedUpCocaLeafs', function()
	local xPlayer = ESX.GetPlayerFromId(source)
	local xItem   = xPlayer.getInventoryItem('cocaleaf')
...
AddEventHandler('esx_drugs:processCocaine', function()
	if not playersProcessingCocaine[source] then
		...
		playersProcessingCocaine[_source] = ESX.SetTimeout(Config.Delays.CocaineProcessing, function()
			local xPlayer             = ESX.GetPlayerFromId(_source)
			local xCocaLeaf, xCocaine = xPlayer.getInventoryItem('cocaleaf'), xPlayer.getInventoryItem('cocaine')

			-- local xSalts = xPlayer.getInventoryItem('salts') --Working on setting up the required items for this
			...
			elseif xCocaLeaf.count < 3 then
				...
				-- elseif xSalts.count < 3 then -- Working on setting up the required items
				-- 	TriggerClientEvent('esx:showNotification', _source, _U('coca_missingitems'))
			else
				xPlayer.removeInventoryItem('cocaleaf', 5)
...
function CancelProcessing(playerID)
	...
	elseif playersProcessingCocaine[playerID] then
