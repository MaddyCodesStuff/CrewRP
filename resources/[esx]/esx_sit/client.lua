...
Citizen.CreateThread(function()
    while true do
        ...
        if (GetLastInputMethod(2) and IsControlJustPressed(1, 38) and IsControlPressed(1,
                                                                                       21)) and not IsPedInAnyVehicle(playerPed,
                                                                                                                      true) then
...
function sit(object, modelName, data)
    ...
    SetEntityCoords(playerPed, pos.x, pos.y, pos.z - data.verticalOffset, true, false, false, false)
    SetEntityHeading(playerPed, GetEntityHeading(object) + 180.0)
    TriggerEvent('emote:do', 'sitchair')
end