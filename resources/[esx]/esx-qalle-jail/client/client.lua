...
RegisterNetEvent("esx-qalle-jail:lowertime")
 AddEventHandler("esx-qalle-jail:lowertime", function(amount)
...
function UnJail()
    ...
    ESX.Game.Teleport(PlayerPedId(), Config.Teleports["Entrance"])
...
function DeliverPackage(packageId)
    if DoesEntityExist(packageId) then
        AttachEntityToEntity(packageId, PlayerPedId(), GetPedBoneIndex(PlayerPedId(), 28422), 0.0, -0.03, 0.0, 5.0, 0.0,
                             0.0, 1, 1, 0, 1, 0, 1)
    ...
    ESX.UI.Menu.Open(
        'default', GetCurrentResourceName(), 'jail_prison_menu',
        {
            ...
            elements = {
                ...
                { label = "Lower Sentance Time", value = "subtract_time"}
            }
        },
        function(data, menu)
            ...
            if action == "jail_closest_player" then
                ...
                function jailClosestPlayerCallback(value)
                    local jailTime = tonumber(value)

                    if jailTime == nil then
                        ESX.ShowNotification("The time needs to be in minutes!")
                        TriggerEvent('tcrp-input:OpenInput', 'Jail Time (minutes)', 'number', 11,
                                     jailClosestPlayerCallback)
                    else
                        ...
                        if closestPlayer == -1 or closestDistance > 3.0 then
                            ...
                            function jailChooseReasonCallback(reason)
                                if reason == nil then
                                    TriggerEvent('tcrp-input:OpenInput', 'Jail Reason', 'text', 255,
                                                 jailChooseReasonCallback)
                                else
                                    local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                                    ...
                                    if closestPlayer == -1 or closestDistance > 3.0 then
                                        ...
                                        TriggerServerEvent("esx-qalle-jail:jailPlayer",
                                                           GetPlayerServerId(closestPlayer), jailTime, reason)
                                    end

                                end
                            end

                            TriggerEvent('tcrp-input:OpenInput', 'Jail Reason', 'text', 255, jailChooseReasonCallback)
                        end

                    end
                end

                TriggerEvent('tcrp-input:OpenInput', 'Jail Time (minutes)', 'number', 11, jailClosestPlayerCallback)
                ...
                ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
                    ...
                    for i = 1, #playerArray, 1 do
                        table.insert(elements,
                                     { label = "Prisoner: " .. playerArray[i].name .. " | Jail Time: " .. playerArray[i].jailTime .. " minutes", value = playerArray[i].identifier })
                    end
            ...
            elseif action == "subtract_time" then
                ...
                function subtractTimeCallback(amount)
                    local time = tonumber(amount)
                    if jailTime == nil then
                        ESX.ShowNotification("This player is not in jail!")
                    else
                        local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
                        ...
                        if closestPlayer == -1 or closestDistance > 3.0 then
                            ESX.ShowNotification("No players nearby!")
                                ...
                                if tonumber(jailTime) < time and jailTime ~= 0 then
                                    ESX.ShowNotification("The amount needs to be less than the remaining time!")
                                    TriggerEvent("tcrp-input:OpenInput", "Reduce Jail Time (minutes)", 'number' , 3, 
                                     subtractTimeCallback)  
                                else
                                    TriggerServerEvent("esx-qalle-jail:lowertime",  GetPlayerServerId(closestPlayer), time)
                                end
