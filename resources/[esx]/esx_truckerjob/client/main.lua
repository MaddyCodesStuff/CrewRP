...
function SpawnTruck()
    ...
    ClearAreaOfVehicles(Config.VehicleSpawnPoint.Pos.x, Config.VehicleSpawnPoint.Pos.y, Config.VehicleSpawnPoint.Pos.z,
    ...
    truck = CreateVehicle(vehiclehash, Config.VehicleSpawnPoint.Pos.x, Config.VehicleSpawnPoint.Pos.y,
                          Config.VehicleSpawnPoint.Pos.z, 0.0, true, false)
    SetEntityAsMissionEntity(truck, true,
                             true) --Not completely sure if this works, but it's supposed to make the engine not delete this vehicle even if out of player area
    ...
    trailer = CreateVehicle(trailerhash, Config.TrailerSpawnPoint.Pos.x, Config.TrailerSpawnPoint.Pos.y,
                            Config.TrailerSpawnPoint.Pos.z, 0.0, true, false)
    ...
    AttachVehicleToTrailer(truck, trailer,
                           1.1) --3rd parameter is the radius, if you changed the spawn locations for the truck and trailer, there might be a chance that you will have to change the radius too
    ...
    deliveryblip   = AddBlipForCoord(alldeliveries[randomdelivery].posx, alldeliveries[randomdelivery].posy,
...
Citizen.CreateThread(function()
    while true do
        ...
        if (GetDistanceBetweenCoords(coords, Config.Zones.VehicleSpawner.Pos.x, Config.Zones.VehicleSpawner.Pos.y,
                                     Config.Zones.VehicleSpawner.Pos.z,
                                     true) < 3) and PlayerData.job ~= nil and PlayerData.job.name == 'trucker' then
            isInMarker  = true
        ...
        if isTaken == 1 and (GetDistanceBetweenCoords(coords, Config.Zones.MissionAbort.Pos.x,
                                                      Config.Zones.MissionAbort.Pos.y, Config.Zones.MissionAbort.Pos.z,
                                                      true) < 3) then
            isInMarker  = true
            currentZone = 'abort'
            LastZone    = 'abort'
    ...
    while true do
        ...
        for k, v in pairs(Config.Zones) do
            if (v.Type ~= -1 and GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z,
                                                          true) < Config.DrawDistance) and PlayerData.job ~= nil and PlayerData.job.name == 'trucker' then
