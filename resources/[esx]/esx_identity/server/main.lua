...
ESX.RegisterServerCallback('esx_identity:getIdentity', function(source, cb, target)
...
AddEventHandler('esx_identity:setIdentity', function(data, myIdentifiers)
    setIdentity(myIdentifiers.steamid, data, function(callback)
        if callback then
            ...
            TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                               "Failed to set character, try again!!")
        end
    end)
end)
...
TriggerEvent('es:addCommand', 'register', function(source, args, user)
    getCharacters(source, function(data)
        if data.firstname3 ~= '' then
            TriggerClientEvent('chatMessage', source, '[IDENTITY]', { 255, 0, 0 },
                               "You can only have 3 registered characters. Use the ^3/delchar^0  command in order to delete existing characters.")
        else
            TriggerClientEvent('esx_identity:showRegisterIdentity', source, {})
        end
    end)
...
TriggerEvent('es:addGroupCommand', 'char', "user", function(source, args, user)
    getIdentity(source, function(data)
        if data.firstname == '' then
            TriggerClientEvent('chatMessage', source, '[IDENTITY]', { 255, 0, 0 },
                               "You do not have an active character!")
        else
            TriggerClientEvent('chatMessage', source, '[IDENTITY]', { 255, 0, 0 },
                               "Active Character: ^2" .. data.firstname .. " " .. data.lastname .. "^0")
        end
    end)
...
TriggerEvent('es:addGroupCommand', 'charlist', "user", function(source, args, user)
    getCharacters(source, function(data)
        if data.firstname1 ~= '' then
            TriggerClientEvent('chatMessage', source, '[IDENTITY] Character 1:', { 255, 0, 0 },
                               data.firstname1 .. " " .. data.lastname1)

            if data.firstname2 ~= '' then
                TriggerClientEvent('chatMessage', source, '[IDENTITY] Character 2:', { 255, 0, 0 },
                                   data.firstname2 .. " " .. data.lastname2)

                if data.firstname3 ~= '' then
                    TriggerClientEvent('chatMessage', source, '[IDENTITY] Character 3:', { 255, 0, 0 },
                                       data.firstname3 .. " " .. data.lastname3)
                end
            end
        else
            TriggerClientEvent('chatMessage', source, '[IDENTITY]', { 255, 0, 0 },
                               "You have no registered characters. Use the ^3/register^0 command to register a character.")
        end
    end)
end, function(source, args, user)
    TriggerClientEvent('chatMessage', source, "SYSTEM", { 255, 0, 0 }, "Insufficient permissions!")
    ...
    getCharacters(source, function(data)
        if charNumber == 1 then
            ...
            if data.firstname ~= '' then
                updateIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
                    if callback then
                        TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                           "Updated your active character to ^2" .. data.firstname .. " " .. data.lastname .. "^0!")
                    else
                        TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                           "Failed to update your identity, try again!")
                    end
                end)
            else
                TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                   "You don\'t have a character in slot 1!")
            end
        elseif charNumber == 2 then
            ...
            if data.firstname ~= '' then
                ...
                TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                   "You don\'t have a character in slot 2!")
            end
        elseif charNumber == 3 then
            ...
            if data.firstname ~= '' then
                ...
                TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                   "You don\'t have a character in slot 3!")
            end
        else
            TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                               "Failed to update your identity, try again!")
...
TriggerEvent('es:addGroupCommand', 'chardel', "user", function(source, args, user)
        ...
        if charNumber == 1 then
            ...
            if data.firstname ~= '' then
                deleteIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
                    if callback then
                        TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                           "You have deleted ^3" .. data.firstname .. " " .. data.lastname .. "!")
                    else
                        TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                           "Failed to delete the identity, try again!")
                    end
                end)
            else
                TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
            ...
            if data.firstname ~= '' then
                deleteIdentity(GetPlayerIdentifiers(source)[1], data, function(callback)
                    if callback then
                        ...
                        TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
                                           "Failed to delete the character, try again!")
                    end
                end)
            else
                TriggerClientEvent('chatMessage', source, "[IDENTITY]", { 255, 0, 0 },
    ...
    TriggerClientEvent('chatMessage', source, "SYSTEM", { 255, 0, 0 }, "Insufficient permissions!")
             ...
             { help = "Delete a registered character", params = { { name = "char", help = "the character id, ranges from 1-3" } } })
