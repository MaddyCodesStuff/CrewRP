...
--     })
-- end, false)
RegisterCommand('admsg', function(source, args, rawCommand)
  local playerName = GetPlayerName(source)
  local msg        = rawCommand:sub(6)
  local name       = getIdentity(source)
  ...
  if xPlayer.getGroup() == 'superadmin' then
    ...
    TriggerClientEvent('chat:addMessage', -1, {
      template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(41, 41, 41, 1); border-radius: 3px; color: #fff;"><i class="fas fa-user-shield"></i> Admin Message: <br> {1}<br></div>',
      args     = { playerName, msg }
    })
  end
end, false)
...
RegisterCommand('advertisement', function(source, args, rawCommand)
  ...
  print(msg)
  TriggerClientEvent('chat:addMessage', -1, {
    template = '<div style="padding: 0.5vw; margin: 0.5vw; background-color: rgba(214, 168, 0, 1); border-radius: 3px; color: #000;"><i class="fas fa-ad"></i> Advertisement:<br> {1}<br></div>',
    args     = { fal, msg }
  })
end, false)
