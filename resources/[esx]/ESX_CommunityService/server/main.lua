...
TriggerEvent('es:addGroupCommand', 'comserv', 'admin', function(source, args, user)
    ...
    TriggerClientEvent('chat:addMessage', source, { args = { _U('system_msn'), _U('insufficient_permissions') } })
             ...
             { help = _U('give_player_community'), params = { { name = "id", help = _U('target_id') }, { name = "actions", help = _U('action_count_suggested') } } })
_U('system_msn')
...
AddEventHandler('esx_communityservice:checkIfSentenced', function()
    ...
    MySQL.Async.fetchAll('SELECT * FROM communityservice WHERE identifier = @identifier', {
        ...
        if result[1] ~= nil and result[1].actions_remaining > 0 then
            ...
            TriggerClientEvent('esx_communityservice:inCommunityService', _source,
                               tonumber(result[1].actions_remaining))
        end
    end)
end)
