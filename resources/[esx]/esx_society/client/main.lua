...
function OpenBossMenu(society, close, options)
    ...
    if options.balance then
        table.insert(elements, { label = "Account Balance", value = 'check_balance' })
    end
        ...
        for i = 1, #players, 1 do
            if players[i].job.name ~= society then
                table.insert(elements,
                             { label = players[i].name, value = players[i].source, name = players[i].name, identifier = players[i].identifier })
            end
        end
...
function OpenPromoteMenu(society, employee)
    ...
    ESX.TriggerServerCallback('esx_society:getJob', function(job)
        ...
        for i = 1, #job.grades, 1 do
            ...
            table.insert(elements,
                         { label = gradeLabel, value = job.grades[i].grade, selected = (employee.job.grade == job.grades[i].grade) })
        end
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    if PlayerData.job.grade_name == 'boss' and Config.ShowSocietyMoney then
