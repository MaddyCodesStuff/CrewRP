...
function OpenCloakroomMenu()
	...
	local elements  = {
		...
		{ label = _U('officer_wear'), value = 'officer_wear' },
		{ label = _U('police2_wear'), value = 'police2_wear' },
		{ label = _U('formal_wear'), value = 'formal_wear' },
		{ label = _U('ticket_wear'), value = 'ticket_wear' },
		{ label = _U('swat_wear'), value = 'swat_wear' },
		{ label = _U('bomb_wear'), value = 'bomb_wear' },
		{ label = _U('tac_wear'), value = 'tac_wear' },
		{ label = _U('tac1_wear'), value = 'tac1_wear' },
		{ label = _U('county_wear'), value = 'county_wear' },
		{ label = _U('county2_wear'), value = 'county2_wear' },
		{ label = _U('highway_wear'), value = 'highway_wear' },
		{ label = _U('highway2_wear'), value = 'highway2_wear' },
		{ label = _U('hwybike_wear'), value = 'hwybike_wear' },
		{ label = _U('countybike_wear'), value = 'countybike_wear' },
	}


	if Config.EnableNonFreemodePeds then
		table.insert(elements,
					 { label = 'Sheriff wear', value = 'freemode_ped', maleModel = 's_m_y_sheriff_01', femaleModel = 's_f_y_sheriff_01' })
		table.insert(elements,
					 { label = 'Police wear', value = 'freemode_ped', maleModel = 's_m_y_cop_01', femaleModel = 's_f_y_cop_01' })
		table.insert(elements,
					 { label = 'Swat wear', value = 'freemode_ped', maleModel = 's_m_y_swat_01', femaleModel = 's_m_y_swat_01' })
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom',
			...
			data.current.value == 'citizen_wear' or
				...
				data.current.value == 'ticket_wear' or
				data.current.value == 'swat_wear' or
				data.current.value == 'bomb_wear' or
				data.current.value == 'tac_wear' or
				data.current.value == 'tac1_wear' or
				data.current.value == 'county_wear' or
				data.current.value == 'county2_wear' or
				data.current.value == 'countybike_wear' or
				data.current.value == 'highway_wear' or
				data.current.value == 'highway2_wear' or
				data.current.value == 'hwybike_wear'
			then
				...
				if foundSpawnPoint then
					if Config.MaxInService == -1 then
						...
						ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
							...
							if isInService then
								ESX.Game.SpawnVehicle(data.current.model, spawnPoint, spawnPoint.heading,
													  function(vehicle)
														  TaskWarpPedIntoVehicle(PlayerPedId(), vehicle, -1)
														  SetVehicleMaxMods(vehicle)
													  end)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
			...
			if data.current.value == 'jail_menu' then
				local elements = {
					...
					{ label = "Traffic Ticket", value = 'ticket' },
					{ label = "Unpaid Fines", value = 'unpaid_bills' },
				...
				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'citizen_interaction',
					{
						...
						if closestPlayer ~= -1 and closestDistance <= 2.0 then
							...
							if action == 'fine' then
								CustomFine(closestPlayer)
							elseif action == 'ticket' then
								CustomFine(closestPlayer, true)
					...
					'default', GetCurrentResourceName(), 'citizen_interaction',
					{
						...
						if closestPlayer ~= -1 and closestDistance <= 2.0 then
							...
							if action == 'body_search' then
								TriggerServerEvent('esx_policejob:message', GetPlayerServerId(closestPlayer),
												   _U('being_searched'))
							...
							elseif action == 'revive' then
								...
								ESX.TriggerServerCallback('esx_policejob:revive', function(has_item)
									if has_item then
										...
										TriggerServerEvent('esx_policejob:reviveplayer',
			...
			elseif data.current.value == 'vehicle_interaction' then
				...
				if DoesEntityExist(vehicle) then
					...
					table.insert(elements, { label = "Seize", value = 'seize' })
				end

				-- table.insert(elements, {label = _U('repair'),       value = 'repair'})
						...
						if DoesEntityExist(vehicle) then
							if action == 'hijack_vehicle' then
								if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.0) then
									...
									TriggerServerEvent('AS_SimpleGarage:changeLockStatus',
							...
							elseif action == 'impound' then
								...
								TriggerEvent('AS_SimpleGarage-main:ImpoundPolice', vehicle)
							elseif action == 'seize' then
								TriggerEvent('tcrp-policeimpound:impoundVehicle', vehicle)
							end
						else
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search',
						 ...
						 function(data2, menu)
							 ...
							 local itemType = data2.current.itemType
							 local itemName = data2.current.value
							 local amount   = data2.current.amount

							 if data2.current.value ~= nil then
								 TriggerServerEvent('esx_policejob:confiscatePlayerItem', GetPlayerServerId(player),
													itemType, itemName, amount)
...
function CustomFine(player, ticket)
	local fined = false
	player = GetPlayerServerId(player)

	function customFineCallback(value)
		local fine_amount = tonumber(value)

		if fine_amount == nil then
			ESX.ShowNotification("Please specify a dollar value.")
			TriggerEvent('tcrp-input:OpenInput', 'Fine Amount', 'number', 11, customFineCallback)
		else
			local label = 'LSPD Fine'
			if Config.EnablePlayerManagement then
				local dueDate = 60 * 60 * 25 * Config.BillDue
				TriggerServerEvent('esx_billing:sendBill', player, 'society_police',
								   _U('fine_total', label), fine_amount, dueDate)
			else
				TriggerServerEvent('esx_billing:sendBill', player, '',
								   _U('fine_total', label),
								   fine_amount)
			end
			fined = true
	...
	function ticketCallback(value)
		local points = tonumber(value)

		if points == nil then
			ESX.ShowNotification("Please specify a number of points to add to license.")
			TriggerEvent('tcrp-input:OpenInput', 'Points on License', 'number', 2, ticketCallback)
		else
			TriggerServerEvent('tcrp-licensepoints:addPointsByType', player, 'drive', points)
		end
	end
...
function OpenUnpaidBillsMenu(player)
	...
	ESX.TriggerServerCallback('esx_billing:getTargetBills', function(bills)
		for i = 1, #bills, 1 do
			table.insert(elements,
						 { label = bills[i].label .. ' - <span style="color: red;">$' .. bills[i].amount .. '</span>', value = bills[i].id })
		end
	...
	ESX.TriggerServerCallback('esx_policejob:getArmoryWeapons', function(weapons)
		...
		for i = 1, #weapons, 1 do
			if weapons[i].count > 0 then
				table.insert(elements,
							 { label = 'x' .. weapons[i].count .. ' ' .. ESX.GetWeaponLabel(weapons[i].name), value = weapons[i].name })
			end
		end
...
AddEventHandler('esx_phone:loaded', function(phoneNumber, contacts)
	...
	TriggerEvent('esx_phone:addSpecialContact', specialContact.name, specialContact.number,
...
AddEventHandler('esx_policejob:handcuff', function()
	...
	Citizen.CreateThread(function()
		if not IsPedCuffed(playerPed) and not isCuffed and not cuffing then
			isCuffed = true
			...
			TriggerEvent('emote:do', 'handcuffing')
			Citizen.Wait(4500)
			TriggerEvent('emote:do', 'handcuffed')
		...
		elseif not cuffing then
			...
			FreezeEntityPosition(playerPed, false)
			Citizen.Wait(100)
			TriggerEvent('emote:do', 'uncuffed')
			Citizen.Wait(5000)
...
RegisterNetEvent('esx_policejob:playcuff')
AddEventHandler('esx_policejob:playcuff', function(target)
	local playerPed        = PlayerPedId()
	local cuff_animation   = 'arresting_cop_shove_right_short'
	local uncuff_animation = ''

	if not IsEntityPlayingAnim(playerPed, 'mp_arresting', cuff_animation, 3) then
		TriggerEvent('emote:do', 'handcuff')
	end
end)
		...
		if IsPedCuffed(playerPed) then
			...
			if DragStatus.IsDragged then
				...
				if not IsPedSittingInAnyVehicle(targetPed) then
					AttachEntityToEntity(playerPed, targetPed, 11816, 0.54, 0.54, 0.0, 0.0, 0.0, 0.0, false, false,
										 false, false, 2, true)
				else
					DragStatus.IsDragged = false
...
Citizen.CreateThread(function()
	local playerPed = PlayerPedId()
	while true do
		if (IsPedCuffed(playerPed) or isCuffed) and not cuffing then
			TriggerEvent('emote:do', 'handcuffed')
		end
		Citizen.Wait(5000)
	end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedCuffed(playerPed) then
			...
			-- DisableControlAction(0, 34, true) -- S (fault in Keys table!)
			-- DisableControlAction(0, 35, true) -- D (fault in Keys table!)
			...
			for k, v in pairs(Config.PoliceStations) do
				...
				for i = 1, #v.Cloakrooms, 1 do
					if GetDistanceBetweenCoords(coords, v.Cloakrooms[i].x, v.Cloakrooms[i].y, v.Cloakrooms[i].z,
						...
						DrawMarker(Config.MarkerType, v.Cloakrooms[i].x, v.Cloakrooms[i].y, v.Cloakrooms[i].z, 0.0,
								   0.0,
				...
				for i = 1, #v.Armories, 1 do
					if GetDistanceBetweenCoords(coords, v.Armories[i].x, v.Armories[i].y, v.Armories[i].z,
												true) < Config.DrawDistance then
						DrawMarker(Config.MarkerType, v.Armories[i].x, v.Armories[i].y, v.Armories[i].z, 0.0, 0.0,
								   0.0,
				...
				for i = 1, #v.VehicleDeleters, 1 do
					if GetDistanceBetweenCoords(coords, v.VehicleDeleters[i].x, v.VehicleDeleters[i].y,
						...
						DrawMarker(Config.MarkerType, v.VehicleDeleters[i].x, v.VehicleDeleters[i].y,
								   ...
								   Config.MarkerSize.y, Config.MarkerSize.z, Config.MarkerColor.r,
				...
				if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss' then
					for i = 1, #v.BossActions, 1 do
						if not v.BossActions[i].disabled and GetDistanceBetweenCoords(coords, v.BossActions[i].x,
																					  ...
																					  v.BossActions[i].z,
																					  true) < Config.DrawDistance then
							DrawMarker(Config.MarkerType, v.BossActions[i].x, v.BossActions[i].y,
									   v.BossActions[i].z,
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
			...
			for k, v in pairs(Config.PoliceStations) do
				...
				if Config.EnablePlayerManagement and PlayerData.job.grade_name == 'boss' then
					for i = 1, #v.BossActions, 1 do
						if GetDistanceBetweenCoords(coords, v.BossActions[i].x, v.BossActions[i].y,
													v.BossActions[i].z,
													true) < Config.MarkerSize.x then
							isInMarker     = true
							currentStation = k
	...
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlJustReleased(0,
									 Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'police' then
		...
		if IsControlJustReleased(0,
								 Keys['F6']) and not isDead and PlayerData.job ~= nil and PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default',
...
RegisterNetEvent('esx_policejob:IsPlayerPolice')
AddEventHandler('esx_policejob:IsPlayerPolice', function(cb)
	cb(IsPolice())
end)
...
function ImpoundVehicle(vehicle)
	TriggerEvent('esx_advancedgarage:impoundVehicle', vehicle)
...
function openJailMenu(playerid)
	local elements = {
		...
		{ label = "Federal Jail", value = 'FederalJail' },
		{ label = "Release Player", value = 'FreePlayer' },
	}
	ESX.UI.Menu.Open(
		...
		function(data3, menu)
			if data3.current.value ~= "FreePlayer" then
				maxLength = 4
				-- Input not replaced because this menu isn't used
				...
				if string.len(jailtime) >= 1 and tonumber(jailtime) ~= nil then
					TriggerServerEvent('esx_jb_jailer:PutInJail', playerid, data3.current.value,
									   tonumber(jailtime) * 60 * 60)
				else
					return false
				end
			else
...
function SendToCommunityService(player)
	function sendToCommunityServiceCallback(value)
		local community_services_count = tonumber(value)

		if community_services_count == nil then
			ESX.ShowNotification('Invalid services count.')
			TriggerEvent('tcrp-input:OpenInput', 'Community Service Menu', 'number', 11, sendToCommunityServiceCallback)
		else
...
function GetVehicleInFront()
	...
	local rayHandle           = StartShapeTestCapsule(plyCoords.x, plyCoords.y, plyCoords.z, plyOffset.x,
													  plyOffset.y,
													  plyOffset.z, 1.0, 2, GetPlayerPed(-1), 4)
