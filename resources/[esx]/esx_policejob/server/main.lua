...
AddEventHandler('onMySQLReady', function()
	MySQL.Async.execute("DELETE FROM `addon_inventory_items` WHERE `inventory_name` = 'society_police'", {})
end)
...
AddEventHandler('esx_policejob:confiscatePlayerItem', function(target, itemType, itemName, amount)
		...
		if targetItem.count > 0 and targetItem.count <= amount then
			...
			if sourceItem.limit ~= -1 and (sourceItem.count + amount) > sourceItem.limit then
				...
				TriggerClientEvent('esx:showNotification', _source,
								   _U('you_confiscated', amount, sourceItem.label, targetXPlayer.name))
				TriggerClientEvent('esx:showNotification', target,
								   _U('got_confiscated', amount, sourceItem.label, sourceXPlayer.name))
			end
		else
			TriggerClientEvent('esx:showNotification', _source, _U('quantity_invalid'))
		end
	...
	elseif itemType == 'item_account' then
		...
		TriggerClientEvent('esx:showNotification', _source,
						   _U('you_confiscated_account', amount, itemName, targetXPlayer.name))
		TriggerClientEvent('esx:showNotification', target,
						   _U('got_confiscated_account', amount, itemName, sourceXPlayer.name))
...
AddEventHandler('esx_policejob:handcuff', function(target)
	...
	if xPlayer.job.name == 'police' or xPlayer.job.name == 'corrections' then
		TriggerClientEvent('esx_policejob:playcuff', source, target)
...
AddEventHandler('esx_policejob:spawned', function()
	...
	if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'police' then
		Citizen.Wait(5000)
		TriggerEvent('eblips:add', { name = GetPlayerName(_source), src = _source, color = 38 })
	end
end)
...
ESX.RegisterServerCallback('esx_policejob:revive', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'police' then
		if item.count < 1 then
			TriggerClientEvent('mythic_notify:client:SendErrorAlert', source,
							   { text = 'You need a medical kit to revive.' })
			cb(false)
		else
...
RegisterServerEvent('esx_policejob:reviveplayer')
AddEventHandler('esx_policejob:reviveplayer', function(target)
