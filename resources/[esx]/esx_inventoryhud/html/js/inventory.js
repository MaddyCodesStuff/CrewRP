...
window.addEventListener('message', function (event) {
    if (event.data.action == 'display') {
        ...
        $.each(event.data.players, function (index, player) {
            $('#nearPlayers').append(
                '<button class="nearbyPlayerButton" data-player="' + player.player + '">' + player.label + ' (' +
...
function inventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        $('#playerInventory').append(
            '<div class="slot"><div id="item-' + index + '" class="item" style = "background-image: url(\'img/items/' +
            item.name + '.png\')">' + '<div class="item-count">' + count + '</div> <div class="item-name">' +
            item.label + '</div> </div ><div class="item-name-bg"></div></div>');
...
function secondInventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        $('#otherInventory').append('<div class="slot"><div id="itemOther-' + index +
                                    '" class="item" style = "background-image: url(\'img/items/' + item.name +
                                    '.png\')">' + '<div class="item-count">' + count +
                                    '</div> <div class="item-name">' + item.label +
...
$(document).ready(function () {
    ...
    $('#playerInventory').droppable({
                                        drop: function (event, ui) {
                                            ...
                                            if (type === 'trunk' && itemInventory === 'second') {
                                            ...
                                            } else if (type === 'housing_property' && itemInventory === 'second') {
                                                disableInventory(500);
                                                ...
                                                disableInventory(500);
                                                $.post('http://esx_inventoryhud/TakeFromDumpster', JSON.stringify({
                                                                                                                      item:   itemData,
                                                                                                                      number: parseInt(
                                                                                                                          $('#count')
                                                                                                                              .val()),
    ...
    $('#otherInventory').droppable({
                                       drop: function (event, ui) {
                                           ...
                                           if (type === 'trunk' && itemInventory === 'main') {
                                               ...
                                               disableInventory(500);
                                               $.post('http://esx_inventoryhud/PutIntoDumpster', JSON.stringify({
                                                                                                                    item:   itemData,
                                                                                                                    id:     dumpster,
                                                                                                                    number: parseInt(
                                                                                                                        $('#count')
                                           ...
                                           } else if (type === 'housing_property' && itemInventory === 'main') {
                                               disableInventory(500);
                                               ...
                                               disableInventory(500);
                                               $.post('http://esx_inventoryhud/PutIntoPersonalProperty',
                                                      JSON.stringify({
                                                                         item:   itemData,
                                                                         number: parseInt($('#count').val())
                                                                     }));
...
$.widget('ui.dialog', $.ui.dialog, {
    options: {
        ...
        clickOutside:        false, // Element (id or class) that triggers the dialog opening
        clickOutsideTrigger: ''
    },
    open:    function () {
