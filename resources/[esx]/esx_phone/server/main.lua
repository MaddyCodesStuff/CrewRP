...
AddEventHandler('esx:playerLoaded', function(source)
  ...
  for num, v in pairs(PhoneNumbers) do
    if tonumber(num) == num then
      -- If phonenumber is a player phone number
  ...
  if PhoneNumbers[phoneNumber] ~= nil then
    ...
    for k, v in pairs(PhoneNumbers[phoneNumber].sources) do
      ...
      if numHasDispatch then
        TriggerClientEvent('esx_phone:onMessage', numSource, xPlayer.get('phoneNumber'), message, numPosition,
                           (numHide and true or anon), numType, GetDistpatchRequestId())
      else
        TriggerClientEvent('esx_phone:onMessage', numSource, xPlayer.get('phoneNumber'), message, numPosition,
                           (numHide and true or anon), numType, false)
      end
  ...
  MySQL.Async.fetchAll(
    'SELECT phone_number FROM users WHERE phone_number = @number',
    {
      ...
      if result[1] ~= nil then
        ...
        if phoneNumber == tonumber(xPlayer.get('phoneNumber')) then
