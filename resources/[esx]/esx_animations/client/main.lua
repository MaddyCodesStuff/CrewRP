...
function OpenAnimationsMenu()
    ...
    table.insert(elements, { label = 'CB Radio', type = '', value = 'radio' })
        ...
        if Config.Animations[i].name == menu then
            ...
            for j = 1, #Config.Animations[i].items, 1 do
                table.insert(elements,
                             { label = Config.Animations[i].items[j].label, type = Config.Animations[i].items[j].type, value = Config.Animations[i].items[j].data })
            end
...
Citizen.CreateThread(function()
    while true do
        ...
        --if IsControlJustReleased(0, Keys['F3']) and not IsPedInAnyVehicle(playerPed) then
        --	OpenAnimationsMenu()
        --end
