Config = {

    -- Theft stuff
    CopsRequired       = 1,
    CopBlipTime        = 120000, -- in sec
    CopBlipInterval    = 5, -- in sec
    MaxItemsStolen     = 10,

    DefaultSprite      = 52,
    DefaultSpriteColor = 11,
    DefaultScale       = 1.0,
    DefaultSpriteName  = "Shop",
    DefaultRobDist     = 20.0,
    DefaultLayer       = 9,

    -- Store config
    Locations          = {
        -- Done
        -- 24/7 @ Innocence Blvd
        [1]  = {
            ["shelfs"]   = {
                { ["x"] = 25.73, ["y"] = -1347.27, ["z"] = 29.5, ["value"] = "checkout" },
                { ["x"] = 27.50, ["y"] = -1345.25, ["z"] = 29.5, ["value"] = "drinks" },
                { ["x"] = 28.27, ["y"] = -1342.76, ["z"] = 29.5, ["value"] = "diverse" },
                { ["x"] = 32.45, ["y"] = -1342.96, ["z"] = 29.5, ["value"] = "readymeal" },
                { ["x"] = 25.67, ["y"] = -1344.99, ["z"] = 29.5, ["value"] = "snacks" },
                { ["x"] = 29.51, ["y"] = -1342.78, ["z"] = 29.5, ["value"] = "chemicals" },
                { ["x"] = 26.53, ["y"] = -1348.09, ["z"] = 29.5, ["value"] = "fruits" },
                { ["x"] = 30.95, ["y"] = -1347.12, ["z"] = 29.5, ["value"] = "donuts" },
            }
        , ["blip"]       = {
                ["scale"] = 0.8,
                ["x"] = 29.41, ["y"] = -1345.01, ["z"] = 29.5
            },
            ["cashier"]  = {
                ["x"] = 24.44, ["y"] = -1347.34, ["z"] = 28.5, ["h"] = 270.82
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- LTD @ Grove St
        [2]  = {
            ["shelfs"]   = {
                { ["x"] = -48.37, ["y"] = -1757.93, ["z"] = 29.42, ["value"] = "checkout" },
                { ["x"] = -48.34, ["y"] = -1751.99, ["z"] = 29.42, ["value"] = "drinks" },
                { ["x"] = -49.5, ["y"] = -1758.32, ["z"] = 29.42, ["value"] = "snacks" },
                { ["x"] = -50.08, ["y"] = -1749.24, ["z"] = 29.42, ["value"] = "readymeal" },
                { ["x"] = -47.25, ["y"] = -1756.58, ["z"] = 29.42, ["value"] = "diverse" },
                { ["x"] = -54.95, ["y"] = -1753.58, ["z"] = 29.42, ["value"] = "fruits" },
                { ["x"] = -50.67, ["y"] = -1754.7, ["z"] = 29.42, ["value"] = "donuts" },
                { ["x"] = -52.7, ["y"] = -1753.39, ["z"] = 29.42, ["value"] = "chemicals" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -48.34, ["y"] = -1752.72, ["z"] = 29.42
            },
            ["rob_dist"] = 14.0,
        }, -- Done

        -- Rob's Liqour @ San Andreas Ave
        [3]  = {
            ["shelfs"]   = {
                { ["x"] = -1222.98, ["y"] = -906.86, ["z"] = 12.33, ["value"] = "checkout" },
                { ["x"] = -1222.04, ["y"] = -906.73, ["z"] = 12.33, ["value"] = "snacks" },
                { ["x"] = -1224.09, ["y"] = -908.13, ["z"] = 12.33, ["value"] = "diverse" },
                { ["x"] = -1226.99, ["y"] = -906.76, ["z"] = 12.33, ["value"] = "drinks" },
                { ["x"] = -1222.61, ["y"] = -903.37, ["z"] = 12.33, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -1220.78, ["y"] = -909.19, ["z"] = 12.33
            },
            ["cashier"]  = {
                ["x"]    = -1221.47, ["y"] = -907.99, ["z"] = 11.36, ["h"] = 28.09,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- Rob's Liqour @ Prosperity St
        [4]  = {
            ["shelfs"]   = {
                { ["x"] = -1487.62, ["y"] = -378.60, ["z"] = 40.16, ["value"] = "checkout" },
                { ["x"] = -1486.07, ["y"] = -380.21, ["z"] = 40.16, ["value"] = "diverse" },
                { ["x"] = -1486.18, ["y"] = -382.6, ["z"] = 40.16, ["value"] = "drinks" },
                { ["x"] = -1487.04, ["y"] = -379.56, ["z"] = 40.16, ["value"] = "snacks" },
                { ["x"] = -1490.8, ["y"] = -379.48, ["z"] = 40.16, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -1485.59, ["y"] = -376.7, ["z"] = 40.16
            },
            ["cashier"]  = {
                ["x"]    = -1486.75, ["y"] = -377.51, ["z"] = 39.18, ["h"] = 130.0,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- LTD @ Little Seoul
        [5]  = {
            ["shelfs"]   = {
                { ["x"] = -707.31, ["y"] = -914.66, ["z"] = 19.22, ["value"] = "checkout" },
                { ["x"] = -710.89, ["y"] = -909.81, ["z"] = 19.22, ["value"] = "drinks" },
                { ["x"] = -713.68, ["y"] = -913.90, ["z"] = 19.22, ["value"] = "chemicals" },
                { ["x"] = -714.20, ["y"] = -909.15, ["z"] = 19.22, ["value"] = "readymeal" },
                { ["x"] = -707.36, ["y"] = -912.83, ["z"] = 19.22, ["value"] = "diverse" },
                { ["x"] = -715.49, ["y"] = -915.54, ["z"] = 19.22, ["value"] = "fruits" },
                { ["x"] = -711.02, ["y"] = -913.6, ["z"] = 19.22, ["value"] = "donuts" },
                { ["x"] = -707.79, ["y"] = -915.68, ["z"] = 19.22, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -711.01, ["y"] = -911.25, ["z"] = 19.22
            },
            ["rob_dist"] = 14.0,
        }, -- Done

        -- Rob's Liqour @ El Rancho Blvd
        [6]  = {
            ["shelfs"]   = {
                { ["x"] = 1135.7, ["y"] = -982.79, ["z"] = 46.42, ["value"] = "checkout" },
                { ["x"] = 1135.3, ["y"] = -980.55, ["z"] = 46.42, ["value"] = "diverse" },
                { ["x"] = 1136.99, ["y"] = -978.76, ["z"] = 46.42, ["value"] = "drinks" },
                { ["x"] = 1135.5, ["y"] = -981.44, ["z"] = 46.42, ["value"] = "snacks" },
                { ["x"] = 1138.69, ["y"] = -984.07, ["z"] = 46.42, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1132.94, ["y"] = -983.19, ["z"] = 46.42
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- 24/7 @ Clinton Ave
        [7]  = {
            ["shelfs"]   = {
                { ["x"] = 373.55, ["y"] = 325.52, ["z"] = 103.57, ["value"] = "checkout" },
                { ["x"] = 375.88, ["y"] = 329.95, ["z"] = 103.57, ["value"] = "drinks" },
                { ["x"] = 378.15, ["y"] = 329.83, ["z"] = 103.57, ["value"] = "chemicals" },
                { ["x"] = 381.29, ["y"] = 328.64, ["z"] = 103.57, ["value"] = "readymeal" },
                { ["x"] = 374.17, ["y"] = 327.92, ["z"] = 103.57, ["value"] = "diverse" },
                { ["x"] = 374.58, ["y"] = 324.81, ["z"] = 103.57, ["value"] = "fruits" },
                { ["x"] = 378.88, ["y"] = 325.06, ["z"] = 103.57, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 378.8, ["y"] = 329.64, ["z"] = 103.57
            },
            ["cashier"]  = {
                ["x"] = 372.54, ["y"] = 326.38, ["z"] = 102.59, ["h"] = 257.27,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- LTD @ Mirror Park
        [8]  = {
            ["shelfs"]   = {
                { ["x"] = 1163.67, ["y"] = -323.92, ["z"] = 69.21, ["value"] = "checkout" },
                { ["x"] = 1159.33, ["y"] = -319.84, ["z"] = 69.21, ["value"] = "drinks" },
                { ["x"] = 1157.31, ["y"] = -324.37, ["z"] = 69.21, ["value"] = "chemicals" },
                { ["x"] = 1156.00, ["y"] = -319.68, ["z"] = 69.21, ["value"] = "readymeal" },
                { ["x"] = 1163.33, ["y"] = -322.25, ["z"] = 69.21, ["value"] = "diverse" },
                { ["x"] = 1163.32, ["y"] = -324.99, ["z"] = 69.21, ["value"] = "snacks" },
                { ["x"] = 1155.89, ["y"] = -326.24, ["z"] = 69.21, ["value"] = "fruits" },
                { ["x"] = 1159.82, ["y"] = -323.55, ["z"] = 69.21, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1157.88, ["y"] = -319.42, ["z"] = 69.21
            },
            ["rob_dist"] = 14.0,
        }, -- Done

        -- 24/7 @ Palomino Fwy
        [9]  = {
            ["shelfs"]   = {
                { ["x"] = 2557.44, ["y"] = 382.03, ["z"] = 108.62, ["value"] = "checkout" },
                { ["x"] = 2553.04, ["y"] = 383.32, ["z"] = 108.62, ["value"] = "drinks" },
                { ["x"] = 2552.65, ["y"] = 385.58, ["z"] = 108.62, ["value"] = "chemicals" },
                { ["x"] = 2553.23, ["y"] = 389.04, ["z"] = 108.62, ["value"] = "readymeal" },
                { ["x"] = 2555.08, ["y"] = 382.18, ["z"] = 108.64, ["value"] = "diverse" },
                { ["x"] = 2556.15, ["y"] = 382.09, ["z"] = 108.62, ["value"] = "snacks" },
                { ["x"] = 2558.26, ["y"] = 382.91, ["z"] = 108.62, ["value"] = "fruits" },
                { ["x"] = 2557.25, ["y"] = 387.29, ["z"] = 108.62, ["value"] = "donuts" },


            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 2552.75, ["y"] = 386.28, ["z"] = 108.62
            },
            ["cashier"]  = {
                ["x"] = 2557.27, ["y"] = 380.81, ["z"] = 107.64, ["h"] = 0.0,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- 24/7 @ Ineseno Rd
        [10] = {
            ["shelfs"]   = {
                { ["x"] = -3039.16, ["y"] = 585.71, ["z"] = 7.91, ["value"] = "checkout" },
                { ["x"] = -3041.83, ["y"] = 586.86, ["z"] = 7.91, ["value"] = "drinks" },
                { ["x"] = -3044.86, ["y"] = 587.45, ["z"] = 7.91, ["value"] = "snacks" },
                { ["x"] = -3045.56, ["y"] = 590.78, ["z"] = 7.91, ["value"] = "readymeal" },
                { ["x"] = -3041.03, ["y"] = 585.11, ["z"] = 7.91, ["value"] = "diverse" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -3042.26, ["y"] = 588.5, ["z"] = 7.91
            },
            ["cashier"]  = {
                ["x"] = -3038.96, ["y"] = 584.53, ["z"] = 6.93, ["h"] = 0.0,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- 24/7 @ Barareno Rd
        [11] = {
            ["shelfs"]   = {
                { ["x"] = -3242.11, ["y"] = 1001.20, ["z"] = 12.83, ["value"] = "checkout" },
                { ["x"] = -3246.29, ["y"] = 1002.57, ["z"] = 12.83, ["value"] = "drinks" },
                { ["x"] = -3246.58, ["y"] = 1004.95, ["z"] = 12.83, ["value"] = "chemicals" },
                { ["x"] = -3245.88, ["y"] = 1008.5, ["z"] = 12.83, ["value"] = "readymeal" },
                { ["x"] = -3244.23, ["y"] = 1001.37, ["z"] = 12.84, ["value"] = "diverse" },
                { ["x"] = -3243.17, ["y"] = 1001.28, ["z"] = 12.83, ["value"] = "snacks" },
                { ["x"] = -3241.13, ["y"] = 1001.92, ["z"] = 12.83, ["value"] = "fruits" },
                { ["x"] = -3241.91, ["y"] = 1006.63, ["z"] = 12.83, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -3246.45, ["y"] = 1005.64, ["z"] = 12.83
            },
            ["cashier"]  = {
                ["x"] = -3242.24, ["y"] = 1000.0, ["z"] = 11.85, ["h"] = 353.5,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- Rob's Liqour @ Great Ocean Hwy
        [12] = {
            ["shelfs"]   = {
                { ["x"] = -2967.78, ["y"] = 391.49, ["z"] = 15.04, ["value"] = "checkout" },
                { ["x"] = -2967.87, ["y"] = 389.3, ["z"] = 15.04, ["value"] = "diverse" },
                { ["x"] = -2969.67, ["y"] = 387.84, ["z"] = 15.04, ["value"] = "drinks" },
                { ["x"] = -2967.88, ["y"] = 390.33, ["z"] = 15.04, ["value"] = "snacks" },
                { ["x"] = -2970.39, ["y"] = 393.39, ["z"] = 15.04, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -2964.96, ["y"] = 391.33, ["z"] = 15.04
            },
            ["cashier"]  = {
                ["x"]    = -2966.38, ["y"] = 391.44, ["z"] = 14.06, ["h"] = 91.62,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- LTD @ North Rockford Dr
        [13] = {
            ["shelfs"]   = {
                { ["x"] = -1820.38, ["y"] = 792.69, ["z"] = 138.11, ["value"] = "checkout" },
                { ["x"] = -1826.19, ["y"] = 793.81, ["z"] = 138.33, ["value"] = "drinks" },
                { ["x"] = -1820.05, ["y"] = 791.6, ["z"] = 138.11, ["value"] = "snacks" },
                { ["x"] = -1829.13, ["y"] = 792.0, ["z"] = 138.26, ["value"] = "readymeal" },
                { ["x"] = -1821.55, ["y"] = 793.97, ["z"] = 138.12, ["value"] = "diverse" },
                { ["x"] = -1824.41, ["y"] = 792.67, ["z"] = 138.18, ["value"] = "chemicals" },
                { ["x"] = -1823.72, ["y"] = 790.66, ["z"] = 138.18, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -1827.64, ["y"] = 793.31, ["z"] = 138.22
            },
            ["rob_dist"] = 14.0,
        }, -- Done

        -- 24/7 @ Route 68
        [14] = {
            ["shelfs"]   = {
                { ["x"] = 547.75, ["y"] = 2671.53, ["z"] = 42.16, ["value"] = "checkout" },
                { ["x"] = 547.23, ["y"] = 2666.77, ["z"] = 42.16, ["value"] = "drinks" },
                { ["x"] = 545.17, ["y"] = 2666.05, ["z"] = 42.16, ["value"] = "chemicals" },
                { ["x"] = 541.8, ["y"] = 2666.06, ["z"] = 42.16, ["value"] = "readymeal" },
                { ["x"] = 548.08, ["y"] = 2669.36, ["z"] = 42.16, ["value"] = "diverse" },
                { ["x"] = 547.98, ["y"] = 2670.46, ["z"] = 42.16, ["value"] = "snacks" },
                { ["x"] = 546.8, ["y"] = 2672.12, ["z"] = 42.16, ["value"] = "fruits" },
                { ["x"] = 542.55, ["y"] = 2670.19, ["z"] = 42.16, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 544.43, ["y"] = 2666.07, ["z"] = 42.16
            },
            ["cashier"]  = {
                ["x"] = 549.04, ["y"] = 2671.36, ["z"] = 41.18, ["h"] = 98.25,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- Scoops Liqour Barn @ Route 68
        [15] = {
            ["shelfs"]   = {
                { ["x"] = 1165.36, ["y"] = 2709.45, ["z"] = 38.16, ["value"] = "checkout" },
                { ["x"] = 1167.64, ["y"] = 2709.41, ["z"] = 38.16, ["value"] = "diverse" },
                { ["x"] = 1169.22, ["y"] = 2707.77, ["z"] = 38.16, ["value"] = "drinks" },
                { ["x"] = 1166.47, ["y"] = 2709.44, ["z"] = 38.16, ["value"] = "snacks" },
                { ["x"] = 1163.81, ["y"] = 2706.46, ["z"] = 38.16, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1167.02, ["y"] = 2711.82, ["z"] = 38.16
            },
            ["cashier"]  = {
                ["x"]    = 1165.29, ["y"] = 2710.79, ["z"] = 37.18, ["h"] = 176.18,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- 24/7 @ Senora Fwy
        [16] = {
            ["shelfs"]   = {
                { ["x"] = 2678.82, ["y"] = 3280.36, ["z"] = 55.24, ["value"] = "checkout" },
                { ["x"] = 2675.5, ["y"] = 3283.52, ["z"] = 55.24, ["value"] = "drinks" },
                { ["x"] = 2676.17, ["y"] = 3285.7, ["z"] = 55.24, ["value"] = "chemicals" },
                { ["x"] = 2678.1, ["y"] = 3288.43, ["z"] = 55.24, ["value"] = "readymeal" },
                { ["x"] = 2676.91, ["y"] = 3281.38, ["z"] = 55.24, ["value"] = "diverse" },
                { ["x"] = 2677.85, ["y"] = 3280.89, ["z"] = 55.24, ["value"] = "snacks" },
                { ["x"] = 2679.84, ["y"] = 3280.66, ["z"] = 55.24, ["value"] = "fruits" },
                { ["x"] = 2681.12, ["y"] = 3285.4, ["z"] = 55.24, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 2676.55, ["y"] = 3286.27, ["z"] = 55.24
            },
            ["cashier"]  = {
                ["x"] = 2678.1, ["y"] = 3279.4, ["z"] = 54.26, ["h"] = 331.07,
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- 24/7 @ Alhambra Dr
        [17] = {
            ["shelfs"]   = {
                { ["x"] = 1961.17, ["y"] = 3740.5, ["z"] = 32.34, ["value"] = "checkout" },
                { ["x"] = 1959.97, ["y"] = 3744.9, ["z"] = 32.34, ["value"] = "drinks" },
                { ["x"] = 1961.68, ["y"] = 3746.29, ["z"] = 32.34, ["value"] = "chemicals" },
                { ["x"] = 1964.74, ["y"] = 3747.71, ["z"] = 32.34, ["value"] = "readymeal" },
                { ["x"] = 1960.18, ["y"] = 3742.21, ["z"] = 32.36, ["value"] = "diverse" },
                { ["x"] = 1960.75, ["y"] = 3741.37, ["z"] = 32.34, ["value"] = "snacks" },
                { ["x"] = 1962.46, ["y"] = 3740.18, ["z"] = 32.34, ["value"] = "fruits" },
                { ["x"] = 1965.76, ["y"] = 3743.45, ["z"] = 32.34, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["x"] = 1962.33, ["y"] = 3746.67, ["z"] = 32.34,
                ["scale"] = 0.8,
                ["layer"] = 8,
            },
            ["cashier"]  = {
                ["scale"] = 0.8,
                ["x"] = 1960.13, ["y"] = 3739.94, ["z"] = 31.36, ["h"] = 297.89
            },
            ["rob_dist"] = 13.0,
        }, -- Done

        -- Liqour Ace @ Algonquin Blvd
        [18] = {
            ["shelfs"]   = {
                { ["x"] = 1393.13, ["y"] = 3605.2, ["z"] = 34.98, ["value"] = "checkout" },
                { ["x"] = 1390.93, ["y"] = 3604.4, ["z"] = 35.0, ["value"] = "diverse" },
                { ["x"] = 1398.46, ["y"] = 3607.04, ["z"] = 35.0, ["value"] = "drinks" },
                { ["x"] = 1392.01, ["y"] = 3604.81, ["z"] = 35.0, ["value"] = "snacks" },
                { ["x"] = 1389.56, ["y"] = 3602.59, ["z"] = 35.0, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1391.23, ["y"] = 3609.29, ["z"] = 34.98
            },
            ["cashier"]  = {
                ["x"]    = 1392.74, ["y"] = 3606.35, ["z"] = 34.0, ["h"] = 202.73,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 17.0,
        }, -- Done

        -- LTD @ Grapeseed
        [19] = {
            ["shelfs"]   = {
                { ["x"] = 1697.92, ["y"] = 4924.46, ["z"] = 42.06, ["value"] = "checkout" },
                { ["x"] = 1703.97, ["y"] = 4924.57, ["z"] = 42.06, ["value"] = "drinks" },
                { ["x"] = 1702.28, ["y"] = 4928.93, ["z"] = 42.06, ["value"] = "chemicals" },
                { ["x"] = 1706.43, ["y"] = 4927.02, ["z"] = 42.06, ["value"] = "readymeal" },
                { ["x"] = 1699.44, ["y"] = 4923.41, ["z"] = 42.06, ["value"] = "diverse" },
                { ["x"] = 1697.42, ["y"] = 4925.54, ["z"] = 42.06, ["value"] = "snacks" },
                { ["x"] = 1701.63, ["y"] = 4931.48, ["z"] = 42.06, ["value"] = "fruits" },
                { ["x"] = 1700.9, ["y"] = 4926.93, ["z"] = 42.06, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1705.22, ["y"] = 4925.39, ["z"] = 42.06
            },
            ["rob_dist"] = 14.0,
        }, -- Done

        -- 24/7 @ Paleto Great Ocean
        [20] = {
            ["shelfs"]   = {
                { ["x"] = 1728.78, ["y"] = 6414.41, ["z"] = 35.04, ["value"] = "checkout" },
                { ["x"] = 1732.01, ["y"] = 6417.9, ["z"] = 35.04, ["value"] = "drinks" },
                { ["x"] = 1733.92, ["y"] = 6417.4, ["z"] = 35.04, ["value"] = "chemicals" },
                { ["x"] = 1736.88, ["y"] = 6415.61, ["z"] = 35.04, ["value"] = "readymeal" },
                { ["x"] = 1729.82, ["y"] = 6416.42, ["z"] = 35.04, ["value"] = "diverse" },
                { ["x"] = 1729.33, ["y"] = 6415.54, ["z"] = 35.04, ["value"] = "snacks" },
                { ["x"] = 1729.22, ["y"] = 6413.47, ["z"] = 35.04, ["value"] = "fruits" },
                { ["x"] = 1733.91, ["y"] = 6412.57, ["z"] = 35.04, ["value"] = "donuts" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = 1734.64, ["y"] = 6417.04, ["z"] = 35.04
            },
            ["cashier"]  = {
                ["x"] = 1727.87, ["y"] = 6415.25, ["z"] = 34.06, ["h"] = 242.93
            },
            ["rob_dist"] = 13.0,
        },

        -- YouTool @ Senora Fwy
        [21] = {
            ["shelfs"]   = {
                { ["x"] = 2750.21, ["y"] = 3488.44, ["z"] = 55.67, ["value"] = "checkout" },
                { ["x"] = 2748.21, ["y"] = 3484.05, ["z"] = 55.67, ["value"] = "checkout" },
                { ["x"] = 2740.53, ["y"] = 3463.15, ["z"] = 55.67, ["value"] = "checkout" },
                { ["x"] = 2745.69, ["y"] = 3487.72, ["z"] = 55.67, ["value"] = "electronics" },
                { ["x"] = 2736.18, ["y"] = 3465.45, ["z"] = 55.67, ["value"] = "fruits" },
                { ["x"] = 2733.59, ["y"] = 3467.59, ["z"] = 55.67, ["value"] = "drinks" },
                { ["x"] = 2733.0, ["y"] = 3496.95, ["z"] = 55.67, ["value"] = "tools" },
                { ["x"] = 2736.33, ["y"] = 3471.31, ["z"] = 55.67, ["value"] = "chemicals" },
                { ["x"] = 2735.58, ["y"] = 3464.57, ["z"] = 55.67, ["value"] = "donuts" },
                { ["x"] = 2748.72, ["y"] = 3480.79, ["z"] = 55.67, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["x"]      = 2739.43, ["y"] = 3480.25, ["z"] = 55.67,
                ["text"]   = "Hardware Store",
                ["scale"] = 0.8,
                ["sprite"] = 566,
                ["color"]  = 47,
            },
            ["rob_dist"] = 30.0,
        },

        -- Del Vecchio Liqour @ Paleto
        [22] = {
            ["shelfs"]   = {
                { ["x"] = -159.23, ["y"] = 6322.31, ["z"] = 31.6, ["value"] = "checkout" },
                { ["x"] = -160.7, ["y"] = 6325.27, ["z"] = 31.6, ["value"] = "drinks" },
                { ["x"] = -160.19, ["y"] = 6323.26, ["z"] = 31.6, ["value"] = "snacks" },
                { ["x"] = -156.71, ["y"] = 6323.08, ["z"] = 31.6, ["value"] = "booze" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["x"] = -162.50, ["y"] = 6321.38, ["z"] = 31.6
            },
            ["cashier"]  = {
                ["x"] = -160.71, ["y"] = 6320.644, ["z"] = 30.6, ["h"] = 320.92
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- Burgershot @ San Andres Ave
        [23] = {
            ["shelfs"]   = {
                { ["x"] = -1192.56, ["y"] = -894.76, ["z"] = 14.0, ["value"] = "checkout" },
                { ["x"] = -1193.9, ["y"] = -892.81, ["z"] = 14.0, ["value"] = "burgershot2" },
                { ["x"] = -1195.07, ["y"] = -890.99, ["z"] = 14.0, ["value"] = "burgershot" },
                { ["x"] = -1196.67, ["y"] = -891.53, ["z"] = 14.0, ["value"] = "burgershot" },
                { ["x"] = -1196.05, ["y"] = -892.44, ["z"] = 14.0, ["value"] = "burgershot2" },
                { ["x"] = -1195.29, ["y"] = -893.62, ["z"] = 14.0, ["value"] = "checkout" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = -1191.43, ["y"] = -890.27, ["z"] = 14.0,
                ["text"]   = "Restaurant",
            },
            ["cashier"]  = {
                ["x"]    = -1194.01, ["y"] = -895.56, ["z"] = 13.0, ["h"] = 308.71,
                ["hash"] = "u_m_y_burgerdrug_01",
            },
            ["rob_dist"] = 17.0,
        }, -- Done

        -- Coffee Shop @ Eclipse Blvd
        [24] = { -- Cafe Bean
            ["shelfs"]   = {
                { ["x"] = -619.45, ["y"] = 233.36, ["z"] = 81.89, ["value"] = "checkout" },
                { ["x"] = -620.85, ["y"] = 230.15, ["z"] = 81.89, ["value"] = "cafebean" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 66,
                ["x"]      = -627.64, ["y"] = 235.25, ["z"] = 81.88,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 15.0,
        }, -- Done

        -- Digital Den @ Portola Dr
        [25] = { -- Digital Den #1
            ["shelfs"]   = {
                { ["x"] = -658.03, ["y"] = -857.28, ["z"] = 24.49, ["value"] = "checkout" },
                { ["x"] = -656.63, ["y"] = -857.28, ["z"] = 24.49, ["value"] = "electronics" },
            },
            ["blip"]     = {
                ["sprite"] = 521,
                ["scale"] = 0.8,
                ["color"]  = 7,
                ["x"]      = -633.22, ["y"] = -281.99, ["z"] = 35.58,
                ["text"]   = "Digital Den",
            },
            ["rob_dist"] = 18.0,
        }, -- Done

        -- Digital Den @ Vespucci Blvd
        [26] = { -- Digital Den #2
            ["shelfs"]   = {
                { ["x"] = -626.56, ["y"] = -279.99, ["z"] = 35.58, ["value"] = "checkout" },
                { ["x"] = -629.77, ["y"] = -274.59, ["z"] = 35.58, ["value"] = "checkout" },
                { ["x"] = -634.06, ["y"] = -282.56, ["z"] = 35.58, ["value"] = "electronics" },
            },
            ["blip"]     = {
                ["sprite"] = 521,
                ["color"]  = 7,
                ["scale"] = 0.8,
                ["x"]      = -658.82, ["y"] = -858.82, ["z"] = 24.49,
                ["text"]   = "Digital Den",
            },
            ["cashier"]  = {
                ["x"]    = -625.12, ["y"] = -279.26, ["z"] = 34.58, ["h"] = 118.31,
                ["hash"] = "cs_tom",
            },
            ["rob_dist"] = 10.0,
        }, -- Done

        -- Digital Den @ Mirror Park
        [27] = { -- Digital Den #3
            ["shelfs"]   = {
                { ["x"] = 1133.87, ["y"] = -473.23, ["z"] = 66.77, ["value"] = "checkout" },
                { ["x"] = 1135.07, ["y"] = -472.06, ["z"] = 66.77, ["value"] = "electronics" },
            },
            ["blip"]     = {
                ["sprite"] = 521,
                ["color"]  = 7,
                ["scale"] = 0.8,
                ["x"]      = 1131.92, ["y"] = -474.17, ["z"] = 66.72,
                ["text"]   = "Digital Den",
            },
            ["rob_dist"] = 12.0,
        },

        -- Pharmacy @ Alta St
        [28] = { -- Pharmacy #1
            ["shelfs"]   = {
                { ["x"] = 94.96, ["y"] = -230.56, ["z"] = 54.66, ["value"] = "checkout" },
                { ["x"] = 92.64, ["y"] = -229.73, ["z"] = 54.66, ["value"] = "pharmacy" },
            },
            ["blip"]     = {
                ["sprite"] = 403,
                ["color"]  = 59,
                ["x"]      = 92.64, ["y"] = -229.73, ["z"] = 54.66,
                ["text"]   = "Pharmacy",
            },
            ["rob_dist"] = 12.0,
        },

        -- Pharmacy @ Strawberry Ave
        [29] = { -- Pharmacy #2
            ["shelfs"]   = {
                { ["x"] = 301.13, ["y"] = -733.07, ["z"] = 29.37, ["value"] = "checkout" },
                { ["x"] = 301.64, ["y"] = -731.45, ["z"] = 29.35, ["value"] = "pharmacy" },
                { ["x"] = 300.57, ["y"] = -734.38, ["z"] = 29.35, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["sprite"] = 403,
                ["color"]  = 59,
                ["x"]      = 300.10, ["y"] = -733.04, ["z"] = 29.35,
                ["text"]   = "Pharmacy",
            },
            ["rob_dist"] = 12.0,
        },

        -- Pharmacy @ Vespucci Blvd
        [30] = { -- Pharmacy #3
            ["shelfs"]   = {
                { ["x"] = 317.3, ["y"] = -1077.04, ["z"] = 29.48, ["value"] = "checkout" },
                { ["x"] = 325.08, ["y"] = -1077.9, ["z"] = 29.48, ["value"] = "pharmacy" },
                { ["x"] = 318.43, ["y"] = -1077.08, ["z"] = 29.48, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["sprite"] = 403,
                ["color"]  = 59,
                ["x"]      = 322.67, ["y"] = -1078.05, ["z"] = 29.48,
                ["text"]   = "Pharmacy",
            },
            ["rob_dist"] = 20.0,
        },
        [31] = { -- Taco Farmer
            ["shelfs"]   = {
                { ["x"] = 13.05, ["y"] = -1599.85, ["z"] = 29.38, ["value"] = "drinks" },
                { ["x"] = 11.43, ["y"] = -1599.43, ["z"] = 29.38, ["value"] = "tacos" },
                { ["x"] = 9.17, ["y"] = -1605.15, ["z"] = 29.38, ["value"] = "checkout" }
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = 11.54, ["y"] = -1606.48, ["z"] = 29.4,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 15.0,
        },
       [32] = { -- Burgershot @ Bowling Alley
            ["shelfs"]   = {
                { ["x"] = -162.57, ["y"] = -241.77, ["z"] = 44.45, ["value"] = "burgershot" },
                { ["x"] = -159.57, ["y"] = -241.32, ["z"] = 44.45, ["value"] = "checkout" },
            },
            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 17,
                ["x"]      = -162.57, ["y"] = -241.77, ["z"] = 43.45,
                ["text"]   = "Bowling Alley",
            },
            ["rob_dist"] = 20.0,
        }, --]]
        [33] = { -- Lodge Store
            ["shelfs"]   = {
                { ["x"] = -676.79, ["y"] = 5834.67, ["z"] = 17.33, ["value"] = "checkout" },
                { ["x"] = -675.38, ["y"] = 5836.05, ["z"] = 17.33, ["value"] = "diverse" },
                { ["x"] = -679.34, ["y"] = 5841.12, ["z"] = 17.33, ["value"] = "drinks" },
                { ["x"] = -676.97, ["y"] = 5837.41, ["z"] = 17.33, ["value"] = "snacks" },
                { ["x"] = -677.95, ["y"] = 5842.02, ["z"] = 17.33, ["value"] = "booze" },
                { ["x"] = -678.12, ["y"] = 5838.57, ["z"] = 17.33, ["value"] = "readymeal" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["layer"] = 8,
                ["x"] = -675.64, ["y"] = 5837.62, ["z"] = 17.33,
            },
            ["rob_dist"] = 15.0,
        }, -- Done
        [34] = { -- Paleto 24/7
            ["shelfs"]   = {
                { ["x"] = 161.57, ["y"] = 6640.86, ["z"] = 31.71, ["value"] = "checkout" },
                { ["x"] = 162.8, ["y"] = 6642.17, ["z"] = 31.71, ["value"] = "diverse" },
                { ["x"] = 165.22, ["y"] = 6642.82, ["z"] = 31.71, ["value"] = "drinks" },
                { ["x"] = 165.62, ["y"] = 6640.05, ["z"] = 31.71, ["value"] = "snacks" },
                { ["x"] = 169.34, ["y"] = 6638.88, ["z"] = 31.71, ["value"] = "readymeal" },
                { ["x"] = 165.23, ["y"] = 6637.28, ["z"] = 31.71, ["value"] = "donuts" },
                { ["x"] = 167.2, ["y"] = 6641.54, ["z"] = 31.71, ["value"] = "chemicals" },
                { ["x"] = 161.75, ["y"] = 6638.94, ["z"] = 31.71, ["value"] = "fruits" },
            },
            ["blip"]     = {
                ["scale"] = 0.8,
                ["layer"] = 8,
                ["x"] = 167.55, ["y"] = 6641.21, ["z"] = 31.71,
            },
            ["cashier"]  = {
                ["x"]    = 160.47, ["y"] = 6641.73, ["z"] = 30.71, ["h"] = 224.34,
                ["hash"] = "s_m_m_linecook"
            },
            ["rob_dist"] = 25.0,
        }, -- Done
        [35] = { -- Hookies
            ["shelfs"]   = {
                { ["x"] = -2186.90, ["y"] = 4274.79, ["z"] = 49.18, ["value"] = "checkout" },
                { ["x"] = -2183.80, ["y"] = 4271.89, ["z"] = 49.18, ["value"] = "readymeal" },
                { ["x"] = -2184.18, ["y"] = 4267.33, ["z"] = 49.18, ["value"] = "drinks" },
                { ["x"] = -2179.90, ["y"] = 4276.74, ["z"] = 49.18, ["value"] = "booze" },
                { ["x"] = -2187.30, ["y"] = 4269.16, ["z"] = 49.18, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = -2185.58, ["y"] = 4286.43, ["z"] = 49.18,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 22.0,
        }, -- Done
        [36] = { -- Pop's Diner
            ["shelfs"]   = {
                { ["x"] = 1589.72, ["y"] = 6456.56, ["z"] = 26.01, ["value"] = "checkout" },
                { ["x"] = 1587.17, ["y"] = 6458.11, ["z"] = 26.01, ["value"] = "readymeal" },
                { ["x"] = 1592.94, ["y"] = 6455.52, ["z"] = 26.01, ["value"] = "drinks" },
                { ["x"] = 1585.53, ["y"] = 6459.05, ["z"] = 26.01, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = 1590.43, ["y"] = 6458.01, ["z"] = 26.01,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 16.0,
        }, -- Done
        [37] = { --Legion hotdog
            ["shelfs"]   = {
                { ["x"] = 218.02, ["y"] = -868.98, ["z"] = 30.49, ["value"] = "hotdogs" },
                { ["x"] = 218.02, ["y"] = -871.48, ["z"] = 30.49, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = 218.02, ["y"] = -871.48, ["z"] = 30.49,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 13.0,
        }, -- Done
        [38] = { --Pearls
            ["shelfs"]   = {
                { ["x"] = -1839.93, ["y"] = -1188.04, ["z"] = 14.30, ["value"] = "drinks" },
                { ["x"] = -1837.23, ["y"] = -1187.57, ["z"] = 14.30, ["value"] = "snacks" },
                { ["x"] = -1844.63, ["y"] = -1188.11, ["z"] = 14.30, ["value"] = "readymeal" },
                { ["x"] = -1839.99, ["y"] = -1191.50, ["z"] = 14.30, ["value"] = "booze" },
                { ["x"] = -1837.47, ["y"] = -1185.28, ["z"] = 14.30, ["value"] = "checkout" },
                --Upstairs
                { ["x"] = -1835.51, ["y"] = -1194.63, ["z"] = 19.30, ["value"] = "booze" },
                { ["x"] = -1834.71, ["y"] = -1193.29, ["z"] = 19.30, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = -1839.93, ["y"] = -1188.04, ["z"] = 13.30,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 30.0,
        }, -- Done
        [39] = { --Lodge
            ["shelfs"]   = {
                { ["x"] = -695.67, ["y"] = 5787.89, ["z"] = 17.33, ["value"] = "drinks" },
                { ["x"] = -689.70, ["y"] = 5797.55, ["z"] = 17.33, ["value"] = "snacks" },
                { ["x"] = -694.12, ["y"] = 5790.60, ["z"] = 17.33, ["value"] = "readymeal" },
                { ["x"] = -687.19, ["y"] = 5800.35, ["z"] = 17.33, ["value"] = "cafebean" },
                { ["x"] = -689.00, ["y"] = 5798.97, ["z"] = 17.33, ["value"] = "booze" },
                { ["x"] = -688.19, ["y"] = 5796.17, ["z"] = 17.33, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = 93,
                ["color"]  = 47,
                ["x"]      = -688.19, ["y"] = 5796.17, ["z"] = 17.33,
                ["text"]   = "Bar",
            },
            ["rob_dist"] = 23.0,
        }, -- Done
        [40] = { --Coffee Cart
            ["shelfs"]   = {
                { ["x"] = 240.86, ["y"] = -559.46, ["z"] = 43.28, ["value"] = "checkout" },
                { ["x"] = 238.85, ["y"] = -560.47, ["z"] = 43.28, ["value"] = "cafebean" },
            },

            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 31,
                ["x"]      = 238.88, ["y"] = -560.48, ["z"] = 43.28,
                ["text"]   = "Coffee",
            },
            ["rob_dist"] = 10.0,
        }, -- Done
        [41] = { --ICE CREAM STORE
            ["shelfs"]   = {
                { ["x"] = -466.17, ["y"] = -29.80, ["z"] = 45.76, ["value"] = "icecream" },
                { ["x"] = -468.09, ["y"] = -34.68, ["z"] = 45.76, ["value"] = "icecream" },
                { ["x"] = -465.34, ["y"] = -33.78, ["z"] = 45.76, ["value"] = "cafebean" },
                { ["x"] = -469.70, ["y"] = -34.55, ["z"] = 45.76, ["value"] = "donuts" },
                { ["x"] = -466.46, ["y"] = -36.05, ["z"] = 45.76, ["value"] = "drinks" },
                { ["x"] = -465.24, ["y"] = -28.57, ["z"] = 45.76, ["value"] = "checkout" },
                { ["x"] = -465.50, ["y"] = -26.78, ["z"] = 45.76, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 66,
                ["x"]      = -465.24, ["y"] = -28.57, ["z"] = 45.76,
                ["text"]   = "Restaurant",
            },
            ["rob_dist"] = 20.0,
        }, -- Done
        [42] = { --ICE CREAM pillbox
            ["shelfs"]   = {
                { ["x"] = 354.45, ["y"] = -584.14, ["z"] = 28.79, ["value"] = "icecream" },
                { ["x"] = 352.65, ["y"] = -583.48, ["z"] = 28.79, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 53,
                ["x"]      = 352.65, ["y"] = -583.48, ["z"] = 28.79,
                ["text"]   = "Icecream",
            },
            ["rob_dist"] = 20.0,
        }, -- Done
        [43] = { --Prison
            ["shelfs"]   = {
                { ["x"] = 1775.73, ["y"] = 2593.76, ["z"] = 45.72, ["value"] = "drinks" },
                { ["x"] = 1775.90, ["y"] = 2587.79, ["z"] = 45.71, ["value"] = "prison" },
                { ["x"] = 1777.94, ["y"] = 2593.38, ["z"] = 45.72, ["value"] = "readymeal" },
                { ["x"] = 1774.94, ["y"] = 2590.15, ["z"] = 45.72, ["value"] = "checkout" },
                { ["x"] = 1777.12, ["y"] = 2587.89, ["z"] = 45.71, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 0,
                ["x"]      = 1777.12, ["y"] = 2587.89, ["z"] = 45.71,
                ["text"]   = "Prison",
            },
            ["rob_dist"] = 20.0,
        }, -- Done
        [44] = { --ICE CREAM Paleto
            ["shelfs"]   = {
                { ["x"] = -266.01, ["y"] = 6321.90, ["z"] = 32.42, ["value"] = "icecream" },
                { ["x"] = -265.04, ["y"] = 6320.53, ["z"] = 32.42, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 53,
                ["x"]      = -265.04, ["y"] = 6320.53, ["z"] = 32.42,
                ["text"]   = "Icecream",
            },
            ["rob_dist"] = 12.0,
        }, -- Done
        [45] = { -- Paleto Pharmacy #4
            ["shelfs"]   = {
                { ["x"] = -176.57, ["y"] = 6383.21, ["z"] = 31.5, ["value"] = "checkout" },
                { ["x"] = -173.7, ["y"] = 6386.14, ["z"] = 31.5, ["value"] = "pharmacy" },
                { ["x"] = -175.53, ["y"] = 6384.25, ["z"] = 31.5, ["value"] = "snacks" },
            },
            ["blip"]     = {
                ["sprite"] = 403,
                ["color"]  = 59,
                ["x"]      = -176.79, ["y"] = 6386.34, ["z"] = 31.5,
                ["text"]   = "Pharmacy",
            },
            ["rob_dist"] = 13.0,
        }, --Done
        [46] = { -- Flower Shop
            ["shelfs"]   = {
                { ["x"] = -1188.10, ["y"] = -1192.14, ["z"] = 7.79, ["value"] = "checkout" },
                { ["x"] = -1183.39, ["y"] = -1195.02, ["z"] = 7.79, ["value"] = "flowers" },
            },
            ["blip"]     = {
                ["sprite"] = 431,
                ["color"]  = 7,
                ["scale"]  = 1.2,
                ["x"]      = -1183.39, ["y"] = -1195.02, ["z"] = 6.29,
                ["text"]   = "Flower Shop",
            },
            ["rob_dist"] = 13.0,
        }, --Done
        [47] = { --Paleto Hardware
            ["shelfs"]   = {
                { ["x"] = 156.81, ["y"] = 6654.35, ["z"] = 31.67, ["value"] = "checkout" },
                { ["x"] = 160.84, ["y"] = 6650.42, ["z"] = 31.67, ["value"] = "tools" },
                --{ ["x"] = 158.23, ["y"] = 6655.77, ["z"] = 31.67, ["value"] = "lockpicks" },
            },
            ["blip"]     = {
                ["sprite"] = 566,
                ["color"]  = 47,
                ["scale"] = 0.8,
                ["x"]      = 157.41, ["y"] = 6650.10, ["z"] = 31.67,
                ["text"]   = "Hardware Store",
            },
            ["rob_dist"] = 25.0,
        },
        [48] = { --O'Connors Irish Pub
            ["shelfs"]   = {
                { ["x"] = 834.74, ["y"] = -115.23, ["z"] = 79.77, ["value"] = "checkout" },
                { ["x"] = 835.95, ["y"] = -115.90, ["z"] = 79.77, ["value"] = "booze" },
                { ["x"] = 837.64, ["y"] = -116.95, ["z"] = 79.77, ["value"] = "drinks" },
                { ["x"] = 826.76, ["y"] = -110.67, ["z"] = 79.77, ["value"] = "pubfood" },
                
            },
            ["blip"]     = {
                ["sprite"] = 93,
                ["color"]  = 47,
                ["x"]      = 838.11, ["y"] = -112.96, ["z"] = 79.77,
                ["text"]   = "Bar",
            },
            ["rob_dist"] = 30.0,
        },
        [49] = { --GoCart Hotdog
            ["shelfs"]   = {
                { ["x"] = -154.37, ["y"] = -2125.19, ["z"] = 16.71, ["value"] = "hotdogs" },
                { ["x"] = -153.14, ["y"] = -2125.47, ["z"] = 16.71, ["value"] = "checkout" },
            },

            ["blip"]     = {
                ["sprite"] = 127,
                ["color"]  = 4,
                ["x"]      = -154.0, ["y"] = -2125.25, ["z"] = 16.71,
                ["text"]   = "GoCart",
            },
            ["rob_dist"] = 15.0,
        },
        [50] = { --Grapeseed Juice Stand
            ["shelfs"]   = {
                { ["x"] = 2742.21, ["y"] = 4413.25, ["z"] = 48.62, ["value"] = "juice" },
                { ["x"] = 2740.95, ["y"] = 4412.92, ["z"] = 48.62, ["value"] = "checkout" },
            },
            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 4,
                ["x"]      = 2742.21, ["y"] = 4413.25, ["z"] = 48.62,
                ["text"]   = "Juice Stand",
            },
            ["rob_dist"] = 10.0,
        },
        [51] = { --Great Chaparral Juice Stand
            ["shelfs"]   = {
                { ["x"] = -440.69, ["y"] = 1595.90, ["z"] = 358.47, ["value"] = "juice" },
                { ["x"] = -441.63, ["y"] = 1594.86, ["z"] = 358.47, ["value"] = "checkout" },
            },
            ["blip"]     = {
                ["sprite"] = -1,
                ["color"]  = 4,
                ["x"]      = -440.69, ["y"] = 1595.90, ["z"] = 358.47,
                ["text"]   = "Juice Stand",
            },
            ["rob_dist"] = 10.0,
        },
        [52] = { --Rex's Diner
            ["shelfs"]   = {
                { ["x"] = 2547.30, ["y"] = 2594.40, ["z"] = 38.11, ["value"] = "checkout" },
                { ["x"] = 2552.10, ["y"] = 2596.60, ["z"] = 38.11, ["value"] = "drinks2" },
                { ["x"] = 2548.91, ["y"] = 2595.56, ["z"] = 38.11, ["value"] = "pubfood" },
                { ["x"] = 2551.15, ["y"] = 2593.28, ["z"] = 38.11, ["value"] = "booze" },
                { ["x"] = 2550.60, ["y"] = 2596.11, ["z"] = 38.11, ["value"] = "booze" },
                { ["x"] = 2551.05, ["y"] = 2586.05, ["z"] = 38.11, ["value"] = "tacos" },
                { ["x"] = 2551.96, ["y"] = 2586.39, ["z"] = 38.11, ["value"] = "burgers" },
                { ["x"] = 2553.50, ["y"] = 2586.95, ["z"] = 38.11, ["value"] = "hotdogs" },
                { ["x"] = 2555.00, ["y"] = 2589.04, ["z"] = 38.11, ["value"] = "meals" },
                { ["x"] = 2553.36, ["y"] = 2589.61, ["z"] = 38.11, ["value"] = "pizza" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = 2550.51, ["y"] = 2596.12, ["z"] = 38.11,
                ["text"]   = "Rex's Diner",
            },
            ["rob_dist"] = 17.5,
        },
        [53] = { --Limey's Juice & Smoothies
            ["shelfs"]   = {
                { ["x"] = -1248.69, ["y"] = -273.12, ["z"] = 37.48, ["value"] = "checkout" },
                { ["x"] = -1247.56, ["y"] = -274.67, ["z"] = 37.48, ["value"] = "limeysdrinks" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = -1242.74, ["y"] = -278.82, ["z"] = 37.58,
                ["text"]   = "Limey's",
            },
                ["rob_dist"] = 16.5,
            },
        [54] = { --Limey's Juice & Smoothies (Boardwalk Area)
            ["shelfs"]   = {
                { ["x"] = -1253.57, ["y"] = -743.76, ["z"] = 20.83, ["value"] = "checkout" },
                { ["x"] = -1252.59, ["y"] = -743.06, ["z"] = 20.83, ["value"] = "assorteddrinks" },
            },
            ["blip"]     = {
                ["sprite"] = 628,
                ["color"]  = 73,
                ["x"]      = -1242.74, ["y"] = -278.82, ["z"] = 37.58,
                ["text"]   = "Limey's",
            },
                ["rob_dist"] = 16.5,
            },
    },

    --[[

    U can specify the hash of the cashier like this:
        ["cashier"] = {
            ["x"] = 0.0, ["y"] = 0.0, ["z"] = 0.0, ["h"] = 0.0,     (h = heading)
            ["hash"] = "mp_m_freemode_01"
        },
        or ["hash"] = 1885233650
    ]]--

    Locales            = {
        ["checkout"]    = "Cash Register",
        ["booze"]       = "Booze",
        ["drinks"]      = "Drinks",
        ["drinks2"]     = "Drinks",
        ["snacks"]      = "Snacks",
        ["readymeal"]   = "Ready meal",
        ["diverse"]     = "Diverse Stuff",
        ["chemicals"]   = "Chemicals",
        ["fruits"]      = "Fruits",
        ["donuts"]      = "Donuts",
        ["electronics"] = "Electronics",
        ["tools"]       = "Tools",
        ["icecream"]    = "Ice Cream",
        ["burgershot"]  = "Order Meals",
        ["burgershot2"] = "Order Food",
        ["meals"]       = "Cook Meals",
        ["burgers"]     = "Cook Burgers",
        ["cafebean"]    = "Coffee",
        ["tacos"]       = "Cook Tacos",
        ["pizza"]       = "Cook Pizza",
        ["hotdogs"]     = "Hotdogs",
        ["pharmacy"]    = "Pharmacy",
        ["prison"]      = "Prison Cafeteria",
        ["drugs"]       = "Drugs",
        ["flowers"]     = "Flower Shop",
        ["pubfood"]     = "Pub Food",
        ["slushies"]    = "Slushies",
        ["juice"]       = "Juice",
        ["limeys"]      = "Order Food",
        ["lockpicks"]   = "Lockpicks",
        ["limeysdrinks"] = "Quinn's Drinks",
        ["assorteddrinks"] = "Boutique Drinks",
    },

    Items              = {
        ["booze"]       = {
            { label = 'Domestic Beer', item = "beer_1", price = 4 },
            { label = 'Imported Beer', item = "beer_2", price = 4 },
            { label = 'Craft Beer', item = "beer_3", price = 8 },
            { label = 'Bourbon', item = "bourbon", price = 50 },
            { label = 'Rye Whiskey', item = "rye", price = 60 },
            { label = 'Scotch Whisky', item = "scotch", price = 75 },
            { label = 'Tequila', item = "tequila", price = 35 },
            { label = 'Vodka', item = "vodka", price = 60 },
            { label = 'Gin', item = "gin", price = 50 },
            { label = 'Rum', item = "rum", price = 50 },
            { label = 'Bloody Mary', item = "bloodymary", price = 35 },
            { label = 'Dark Ale', item = "darkale", price = 10 },
            { label = 'Pale Ale', item = "paleale", price = 10 },
            { label = 'Tequila Sunrise', item = "tequi-la-sunrise", price = 30 },
        },
        ["drinks"]      = {
            { label = "E-Cola", item = "cocacola", price = 7 },
            { label = "Orange-O-Tang Soda", item = "fanta", price = 7 },
            { label = "Sprunk", item = "sprite", price = 7 },
            { label = "Sprunk Light", item = "sprite2", price = 7 },
            { label = "Water", item = "water", price = 5 },
            { label = "Sludgie Blue Slushy", item = "slushy", price = 3 },
            { label = "Sludgie Green Slushy", item = "slushy2", price = 3 },
            { label = "Milk", item = "milk", price = 3 },
        },
        ["drinks2"]      = {
            { label = "E-Cola", item = "cocacola", price = 5 },
            { label = "Orange-O-Tang Soda", item = "fanta", price = 5 },
            { label = "Sprunk", item = "sprite", price = 5 },
            { label = "Sprunk Light", item = "sprite2", price = 5 },
            { label = "Water", item = "water", price = 5 },
            { label = "Drink", item = "drink", price = 3 },
        },
        ["icecream"]    = {
            { label = "Vanilla Ice Cream Cone", item = "icecream", price = 2 },
            { label = "Chocolate Ice Cream Bowl", item = "icecream3", price = 2 },
            { label = "Mint Ice Cream Cone", item = "icecream2", price = 2 },
            { label = "Milk Shake", item = "milkshake", price = 10 },
            { label = "Ice Cream Sundae", item = "icecream3", price = 10 },
            { label = "Just a Cone", item = "icecream4", price = 1},
        },
        ["snacks"]      = {
            { label = "Big Cheese Chips", item = "cheesebows", price = 2 },
            { label = "SuperSalt Chips", item = "chips", price = 2 },
            { label = "Habanero Chips", item = "chips2", price = 2 },
            { label = "Sticky Ribs Chips", item = "chips3", price = 2 },
            { label = "EarthQuake's Candy Bar", item = "candy", price = 1 },
            { label = "Captain's Log Candy Bar", item = "candy2", price = 1 },
            { label = "Meteorite Candy Bar", item = "candy3", price = 1 },
            { label = "Zebrabar Candy Bar", item = "candy4", price = 1 },
            { label = "Ego Chaser Candy Bar", item = "candy5", price = 1 },
            { label = "Orange-O-Tang Popsicle", item = "popsicle", price = 1 },
            { label = "Creamy Chufty Popsicle", item = "popsicle2", price = 1 },
            { label = "Chocolate Starfish Popsicle", item = "popsicle3", price = 1 },
            { label = "Chilldo Popsicle", item = "popsicle4", price = 1 },
            { label = "Chocolate Chufty Popsicle", item = "popsicle5", price = 1 },
        },
        ["fruits"]      = {
            { label = "Red Apple", item = "redapple", price = 1 },
            { label = "Green Apple", item = "greenapple", price = 1 },
            { label = "Orange", item = "orange", price = 1 },
            { label = "Banana", item = "banana", price = 1 },
            { label = "Pear", item = "pear", price = 1 },
            { label = "Pineapple", item = "pineapple", price = 3 },
        },
        ["readymeal"]   = {
            { label = "Sandwich", item = "bread", price = 5 },
            { label = "Bacon Roll", item = "burger", price = 7 },
            { label = "Pasta Carbonara", item = "pastacarbonara", price = 10 },
            { label = "Pizza", item = "pizza", price = 15 },
            { label = "Chicken Noodles", item = "noodles", price = 4 },
            { label = "Seafood Noodles", item = "noodles2", price = 4 },
            { label = "Tomato Soup", item = "soup", price = 4 },
            { label = "Vegetable Soup", item = "soup2", price = 4 },
            { label = "Crackles O'Dawn Cereal", item = "cereal", price = 4 },
            { label = "Fishy Stix", item = "fishstix", price = 5 },
            { label = "Frozen Waffles", item = "waffles", price = 7 },
        },
        ["diverse"]     = {
            { label = "Redwood Cigarettes", item = "cigarette", price = 6 },
            { label = "Cardiaque Cigarettes", item = "cigarette2", price = 6 },
            { label = "Debonaire Cigarettes", item = "cigarette3", price = 6 },
            { label = "Debonaire Cigarettes Menthol", item = "cigarette4", price = 6 },
            { label = "69 Brand Cigarettes", item = "cigarette5", price = 6 },
            { label = "69 Brand Cigarettes Menthol", item = "cigarette6", price = 6 },
            { label = "Vinewood Scratch Card", item = "coupon", price = 1 },
            { label = "Burgershot Scratch Card", item = "coupon2", price = 1 },
            { label = "Mega Melons Scratch Card", item = "coupon3", price = 1 },
            { label = "Barf Scratch Card", item = "coupon4", price = 1 },
            { label = "Lottery Ticket", item = "lotteryticket", price = 2 },
            { label = "Lax to the Max Pads", item = "maxpads", price = 6 },
            { label = "Nicotine Patches", item = "patches", price = 10 },
            { label = "Long Life Batteries", item = "lithium", price = 6 },
            { label = "Lighter", item = "lighter", price = 1 },
            { label = "Toothpaste", item = "toothpaste", price = 3 },
            { label = "Notepad", item = "notepad", price = 100 },
            { label = "Empty Bottle", item = "empty_bottle", price = 10 },
            { label = "Pet Treats", item = "pettreats", price = 5 },
            { label = "Pet Food", item = "croquettes", price = 150 },
        },
        ["chemicals"]   = {
            { label = "Bleach", item = "bleach", price = 5 },
            { label = "Acetone", item = "acetone", price = 5 },
        },
        ["donuts"]      = {
            { label = "Brown Sprinkle Donut", item = "donut", price = 2 },
            { label = "Birthday Donut", item = "donut2", price = 2 },
        },
        ["electronics"] = {
            { label = "Long Life Batteries", item = "lithium", price = 6 },
            { label = "Cell Phone", item = "phone", price = 500 },
            { label = "Radio", item = "radio", price = 100 },
        },
        ["tools"]       = {
            { label = "Blowtorch Nozzle", item = "blowpipe", price = 10 },
            { label = "Gas Can", item = "gazbottle", price = 10 },
            { label = "Roadside Repair Kit", item = "repairkit_basic", price = 500 },
            { label = "Car Cleaning Kit", item = "carcleaner", price = 150 },
            { label = "Fishing Rod", item = "fishingrod", price = 300 },
            { label = "Fish Bait", item = "fishbait", price = 15 },
            { label = "Jackhammer", item = "jackhammer", price = 2000},
            { label = "Propane Tank", item = "gastank", price = 350},
        },
        ["burgershot"]  = {
            { label = "The Bleeder Burger Meal", item = "bleedermeal", price = 10 },
            { label = "Money Shot Burger Meal", item = "moneyshotmeal", price = 10 },
            { label = "Torpedo Meal", item = "torpedomeal", price = 10 },
            { label = "Meat Free Meal", item = "meatfreemeal", price = 11 },
            { label = "Crappy Meal", item = "crappymeal", price = 12 },
            { label = "Heartstopper Meal", item = "heartstoppermeal", price = 15 },
        },
        ["burgershot2"] = {
            { label = "The Bleeder Burger", item = "bleederburger", price = 6 },
            { label = "Money Shot Burger", item = "moneyshotburger", price = 6 },
            { label = "Torpedo Sandwich", item = "torpedoburger", price = 6 },
            { label = "Meat Free Burger", item = "meatfreeburger", price = 7 },
            { label = "Crappy Burger", item = "crappyburger", price = 8 },
            { label = "The Heartstopper Burger", item = "heartstopperburger", price = 10 },
            { label = "Large Fries", item = "largefries", price = 2 },
            { label = "Water", item = "water", price = 5 },
            { label = "Drink", item = "drink", price = 3 },
        },
        ["burgers"] = {
            { label = "The Bleeder Burger", item = "bleederburger", price = 6 },
            { label = "Money Shot Burger", item = "moneyshotburger", price = 6 },
            { label = "Torpedo Sandwich", item = "torpedoburger", price = 6 },
            { label = "Meat Free Burger", item = "meatfreeburger", price = 7 },
            { label = "Crappy Burger", item = "crappyburger", price = 8 },
            { label = "The Heartstopper Burger", item = "heartstopperburger", price = 10 },
            { label = "Sandwich", item = "bread", price = 5 },
            { label = "Bacon Roll", item = "burger", price = 7 },
        },
        ["meals"] = {
            { label = "The Bleeder Burger Meal", item = "bleedermeal", price = 10 },
            { label = "Money Shot Burger Meal", item = "moneyshotmeal", price = 10 },
            { label = "Torpedo Meal", item = "torpedomeal", price = 10 },
            { label = "Meat Free Meal", item = "meatfreemeal", price = 11 },
            { label = "Crappy Meal", item = "crappymeal", price = 12 },
            { label = "Heartstopper Meal", item = "heartstoppermeal", price = 15 },
            { label = "Hotdog And Fries", item = "hotdogmeal2", price = 8 },
            { label = "Hotdog with the Works Meal", item = "hotdogmeal1", price = 10 },
            { label = "Veggie Dog Meal", item = "veggiedogmeal", price = 10 },
        },
        ["pizza"] = {
            { label = "Pizza", item = "pizza", price = 15 },
        },
        ["cafebean"]    = {
            { label = "Regular Coffee", item = "coffee", price = 1 },
            { label = "Decaf Coffee", item = "decafcoffee", price = 1 },
            { label = "Mocha", item = "mocha", price = 5 },
            { label = "Latte", item = "latte", price = 5 },
            { label = "Frapuccino", item = "frapuccino", price = 5 },
            { label = "Sweet Tea", item = "sweet_tea", price = 5 },
            { label = "Iced Tea", item = "iced_tea", price = 5 },
            { label = "Tea", item = "tea", price = 5 },
            { label = "Hot Cocoa", item = "hotcoco", price = 5 },       
            { label = "Fancy Latte", item = "petcoffee", price = 10 },       
        },
        ["tacos"]       = {
            { label = "Taco Al Pastor", item = "tacoalpastor", price = 5 },
            { label = "Barbacacoa Taco", item = "tacobarbacacoa", price = 5 },
            { label = "Carnitas Taco", item = "tacocarnitas", price = 5 },
            { label = "Veggie Taco", item = "tacovegetal", price = 5 },
            { label = "Enchiladas", item = "enchiladas", price = 5 },
            { label = "Chimichangas", item = "chimichangas", price = 5 },
            { label = "Burrito", item = "burrito", price = 5 },
            { label = "Chips and Guac", item = "chipsandguac", price = 5 },
            { label = "Chips and Salsa", item = "chipsandsalsa", price = 5 },
            { label = "Churro", item = "churro", price = 5 },
            { label = "Quesadillas", item = "quesadillas", price = 5 },
            { label = "Tamale", item = "tamale", price = 5 },
            { label = "Hot Sauce (Do Not Drink)", item = "hotsauce", price = 5 },
        },
        ["pharmacy"]    = {
            { label = "Tylenol", item = "tylenol", price = 10 },
            { label = "Bandage", item = "bandage", price = 6 },
            { label = "Gauze", item = "gauze", price = 7 },
            { label = "Chill Pill", item = "chillpill", price = 25 },
            { label = "Dechill Pill", item = "dechillpill", price = 25},
            { label = "Multi-Vitamin", item = "multi-vitamin", price = 25},
            { label = "Anti-Vitamin", item = "anti-vitamin", price = 25},

        },
        ["prison"]      = {
            { label = "Tomato Soup", item = "soup", price = 4 },
            { label = "Sandwich", item = "bread", price = 5 },
            { label = "Crackles O'Dawn Cereal", item = "cereal", price = 4 },
            { label = "Water", item = "water", price = 5 },
        },
        ["hotdogs"]     = {
            { label = "Plain Hotdog", item = "hotdog1", price = 5 },
            { label = "VeggieDog", item = "hotdog4", price = 5 },
            { label = "Cheese Dog", item = "hotdog2", price = 6 },
            { label = "Hotdog with Onion", item = "hotdog5", price = 6 },
            { label = "Chili Dog", item = "hotdog3", price = 7 },
            { label = "Hotdog And Fries", item = "hotdogmeal2", price = 8 },
            { label = "Hotdog with the Works Meal", item = "hotdogmeal1", price = 10 },
            { label = "Veggie Dog Meal", item = "veggiedogmeal", price = 10 },
        },
        ["hotdogs2"]     = {
            { label = "Plain Hotdog", item = "hotdog1", price = 5 },
            { label = "VeggieDog", item = "hotdog4", price = 5 },
            { label = "Cheese Dog", item = "hotdog2", price = 6 },
            { label = "Hotdog with Onion", item = "hotdog5", price = 6 },
            { label = "Chili Dog", item = "hotdog3", price = 7 },
        },
        ["drugs"]       = {
            { label = "Baking Sheet", item = "baking_sheet", price = 20 },
        },
        ["flowers"]      = {
            { label = "Flower", item = "flower", price = 10 },
            { label = "Red Rose", item = "rosered", price = 20 },
            { label = "White Rose", item = "rosewhite", price = 20 },
            { label = "Bouquet", item = "bouquet", price = 150 },
        },
        ["pubfood"]     = {
            { label = "Plain Hotdog", item = "hotdog1", price = 5 },
            { label = "Hotdog with Fries", item = "hotdogmeal2", price = 8 },
            { label = "VeggieDog", item = "hotdog4", price = 5 },
            { label = "Large Fries", item = "largefries", price = 2 },
            { label = "Money Shot Burger", item = "moneyshotburger", price = 6 },
            { label = "Meat Free Burger", item = "meatfreeburger", price = 7 },
            { label = "Pizza", item = "pizza", price = 15 },
            { label = "Irish Stew", item = "irishstew", price = 20 },        
            { label = "Bangers and Mash", item = "bangersandmash", price = 20 },     
            { label = "Shepherds Pie", item = "shepherdspie", price = 20 },      
        },
        ["juice"]       = {
            { label = "Grape Juice", item = "jus_raisin", price = 5 },
            { label = "Juice", item = "juice", price = 7 },
        },
        ["slushies"]       = {
            { label = "Sludgie Blue Slushy", item = "slushy", price = 3 },
            { label = "Sludgie Green Slushy", item = "slushy2", price = 3 },
        },
        ["limeysdrinks"]       = {
            { label = "Grape Juice", item = "jus_raisin", price = 5 },
            { label = "Juice", item = "juice", price = 7 },
            { label = "Sludgie Blue Slushy", item = "slushy", price = 3 },
            { label = "Sludgie Green Slushy", item = "slushy2", price = 3 },
            { label = "Regular Coffee", item = "coffee", price = 1 },
            { label = "Decaf Coffee", item = "decafcoffee", price = 1 },
            { label = "Sweet Tea", item = "sweet_tea", price = 5 },
            { label = "Iced Tea", item = "iced_tea", price = 5 },
            { label = "Tea", item = "tea", price = 5 },
            { label = "Hot Cocoa", item = "hotcoco", price = 5 },            
        },
        ["assorteddrinks"]       = {
            { label = "Grape Juice", item = "jus_raisin", price = 5 },
            { label = "Juice", item = "juice", price = 7 },
            { label = "Sludgie Blue Slushy", item = "slushy", price = 3 },
            { label = "Sludgie Green Slushy", item = "slushy2", price = 3 },
            { label = "Regular Coffee", item = "coffee", price = 1 },
            { label = "Decaf Coffee", item = "decafcoffee", price = 1 },
            { label = "Sweet Tea", item = "sweet_tea", price = 5 },
            { label = "Iced Tea", item = "iced_tea", price = 5 },
            { label = "Tea", item = "tea", price = 5 },
            { label = "Hot Cocoa", item = "hotcoco", price = 5 },            
        },
        ["lockpicks"]       = {
            { label = "Lockpick", item = "lockpick", price = 500 },
        },
    },
}