...
Citizen.CreateThread(function()
    while true do
        ...
        if isTokovoip then
            SendNUIMessage({
                               action    = 'voice-color',
                               isTalking = exports.tokovoip_script:getPlayerData(GetPlayerServerId(PlayerId()),
                                                                                 'voip:talking')
                           })
        else
            SendNUIMessage({
...
Citizen.CreateThread(function()
    while true do
        TriggerEvent('esx_status:getAllStatus', function(vals)
            local update = { action = "updateStatus" }
            if vals.hunger then
                update.hunger = vals.hunger.getPercent()
            else
                update.hunger = 0
            end
            if vals.thirst then
                update.thirst = vals.thirst.getPercent()
            else
                update.thirst = 0
            end
            if vals.drunk then
                update.drunk = vals.drunk.getPercent()
            else
                update.drunk = 0
            end
            if vals.drug then
                update.high = vals.drug.getPercent()
            else
                update.high = 0
            end
            if vals.poison then
                update.poison = vals.poison.getPercent()
            else
                update.poison = 0
            end
...
AddEventHandler('disc-hud:EnteredVehicle', function()
    ...
    Citizen.CreateThread(function()
        while IsPedInAnyVehicle(player) do
            ...
            if IsControlJustReleased(0, 20) then
                ...
                local maxSpeed = cruiseIsOn and cruiseSpeed or GetVehicleHandlingFloat(veh, "CHandlingData",
    ...
    -- Disable control if seatbelt is On
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(1)
