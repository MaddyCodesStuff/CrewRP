...
function RemovePlayerFromInstance(instance, player)
	if instances[instance] then
		...
		if instances[instance].host == player then
			for i = 1, #instances[instance].players do
				if instances[instance].players[i] ~= player then
					TriggerClientEvent('instance:onPlayerLeft', instances[instance].players[i], instances[instance],
									   player)
				end
			end
...
RegisterCommand('uninstance', function(source, args, rawCommand)
	local player = ESX.GetPlayerFromId(source)
	local playerGroup = player.getGroup()
	if #args > 0 then
		if args[1] == _source then
			TriggerClientEvent('instance:leave', source)
		else
			if playerGroup == "admin" or playerGroup == "superadmin" then
				
			
				TriggerClientEvent('instance:leave', tonumber(args[1]))
			end
		end
	else
		TriggerClientEvent('instance:leave',source)
