...
AddEventHandler(
 "onMySQLReady",
 function()
   MySQL.Async.execute("DELETE FROM `trunk_inventory` WHERE TRIM(`plate`) NOT IN (SELECT TRIM(`plate`) FROM `owned_vehicles`)", {})
 end
)
            ...
            if targetItem.limit == -1 or ((targetItem.count + count) <= targetItem.limit) then
                TriggerEvent(
                    ...
                    function(store)
                        ...
                        TriggerClientEvent("esx_inventoryhud:refreshTrunkInventory", _source, data, blackMoney, items,
                                           weapons)
                    end
                )
            else
