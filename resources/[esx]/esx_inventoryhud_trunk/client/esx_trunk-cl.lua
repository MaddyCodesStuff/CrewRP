...
function openmenuvehicle()
    ...
    if not IsPedInAnyVehicle(playerPed) then
        ...
        for i = 1, #vehiclePlate do
            if vehiclePlate[i] and vehiclePlate[i].plate then
        ...
        if not Config.CheckOwnership or (Config.AllowPolice and PlayerData.job.name == "police") or (Config.CheckOwnership and myVeh) then
            if globalplate ~= nil or globalplate ~= "" or globalplate ~= " " then
                ...
                if GetVehicleNumberPlateText(vehFront) ~= GetVehicleNumberPlateText(closecar) then
                    TriggerEvent('mythic_notify:client:SendErrorAlert',
                                 { text = "Please face the trunk you are attempting to access." })
                elseif vehFront > 0 and closecar ~= nil and GetPedInVehicleSeat(vehFront, -1) ~= GetPlayerPed(-1) then
                    ...
                    if ESX.UI.Menu.IsOpen("default", GetCurrentResourceName(), "inventory") then
                        if GetVehicleDoorAngleRatio(closecar, 5) <= 0.0 then
                        ...
                        if locked == 1 or class == 15 or class == 16 or class == 14 then
                            ...
                            TriggerEvent('mythic_notify:client:SendErrorAlert', { text = "Trunk is locked." })
                        end
                    end
                else
                    TriggerEvent('mythic_notify:client:SendErrorAlert', { text = "No vehicles near by." })
                end
                lastOpen = true
...
function OpenCoffreInventoryMenu(plate, max, myVeh)
    ESX.TriggerServerCallback(
        ...
        function(inventory)
            ...
            TriggerEvent("esx_inventoryhud:openTrunkInventory", data, inventory.blackMoney, inventory.items,
                         inventory.weapons)
        end,
        plate
    )
