...
function HandleMarkers()
	...
	if CurrentStatus ~= Status.DELIVERY_INACTIVE then
		...
		if GetDistanceBetweenCoords(pCoords.x, pCoords.y, pCoords.z, bCoords.x, bCoords.y, bCoords.z, true) < 150.0 then
			...
			DrawMarker(25, ScooterPos.x, ScooterPos.y, ScooterPos.z, 0, 0, 0, 0, 0, 0, 2.5, 2.5, 2.5, 243, 56, 56, 150, false, true)
			DrawMarker(25, VanPos.x, VanPos.y, VanPos.z, 0, 0, 0, 0, 0, 0, 2.5, 2.5, 2.5, 250, 170, 60, 150, false, true)
			DrawMarker(25, TruckPos.x, TruckPos.y, TruckPos.z, 0, 0, 0, 0, 0, 0, 2.5, 2.5, 2.5, 230, 219, 91, 150, false, true)
...
function SpawnDeliveryVehicle(deliveryType)
	...
	local SpawnLocation = { x = -314.30, y = -1023.83, z = 30.39}
...
function EndDelivery()
	local PlayerPed = GetPlayerPed(-1)
	if IsPedSittingInAnyVehicle(PlayerPed) or GetVehiclePedIsIn(PlayerPed) ~= CurrentVehicle then
...
function ReturnVehicle(deliveryType)
	...
	ESX.ShowNotification("Delivery Vehicle Returned")
...
--Cancel the route from anywhere by hitting Del. 
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
