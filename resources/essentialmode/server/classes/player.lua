...
function CreatePlayer(source, permission_level, money, bank, identifier, license, group, roles)
        ...
        if type(m) == "number" then
            ...
            if ((prevMoney - newMoney) < 0) then
                TriggerClientEvent("es:addedMoney", self.source, math.abs(prevMoney - newMoney),
                                   (settings.defaultSettings.nativeMoneySystem == "1"))
            else
    ...
    rTable.addMoney       = function(m)
        if type(m) == "number" then
            ...
            TriggerClientEvent("es:addedMoney", self.source, m, (settings.defaultSettings.nativeMoneySystem == "1"),
                               self.money)
    ...
    rTable.removeMoney    = function(m)
        if type(m) == "number" then
            ...
            TriggerClientEvent("es:removedMoney", self.source, m, (settings.defaultSettings.nativeMoneySystem == "1"),
                               self.money)
    ...
    rTable.giveRole       = function(role)
        ...
        db.updateUser(self.identifier, { roles = table.concat(self.roles, "|") }, function()
        end)
    end
