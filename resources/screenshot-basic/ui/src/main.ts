import {
    CfxTexture, LinearFilter, Mesh, NearestFilter, OrthographicCamera, PlaneBufferGeometry, RGBAFormat, Scene,
    ShaderMaterial, UnsignedByteType, WebGLRenderer, WebGLRenderTarget
...
class ScreenshotUI {
    ...
    initialize() {
        ...
        const cameraRTT: any = new OrthographicCamera(window.innerWidth / -2, window.innerWidth / 2,
        ...
        const material = new ShaderMaterial({
                                                ...
                                                uniforms: {'tDiffuse': {value: gameTexture}}, vertexShader: `
			varying vec2 vUv;
			void main() {
...
`
    ...
    handleRequest(request: ScreenshotRequest) {
        ...
        fetch(request.targetURL, {
            method: 'POST', mode: 'cors', body: (request.targetField) ? getFormData() : JSON.stringify({
                                                                                                           data: imageURL,
                ...
                if (request.resultURL) {
                    fetch(request.resultURL, {
                        method: 'POST', mode: 'cors', body: JSON.stringify({
                                                                               data: text, id: request.correlation
                                                                           })
                    });
                }
            });
