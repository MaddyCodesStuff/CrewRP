...
AddEventHandler("safecracking:start", function(dials, success, store)
	
	TriggerEvent('tcrp:displayGeneral', "Cracking Safe")
	TriggerEvent("safecracking:loop",dials, success, store)
end)

cracking = false
RegisterNetEvent("safecracking:loop")
AddEventHandler("safecracking:loop", function(difficulty,functionName, store, source)
	loadSafeTexture()
	loadSafeAudio()
	difficultySetting = {}
	...
	dicks = 1
	local pinfall = false
	TriggerEvent('tcrp:displayGeneral', "Press Shift+F or F to rotate, H to crack!")
	...
	if curLock > difficulty then
		TriggerServerEvent(functionName, store)
	end
