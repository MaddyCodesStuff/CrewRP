    ...
    while true do
        ...
        for k, v in pairs(Config.Elevators) do
            if GetDistanceBetweenCoords(coords, v.Pos.x, v.Pos.y, v.Pos.z, true) < Config.DrawDistance then
                ...
                local text = 'Press [~g~E~w~] To Access'
...
function OpenElevatorMenu()
    ...
    local elements = {
        { label = 'Select a Destination', value = nil }
    }

    for k, v in pairs(Config.Elevators[elevatormenu].options) do
        table.insert(elements, { label = v.label, value = v.name, pos = v.pos, heading = v.heading })
    end
        ...
        function(data, menu)
            if data.current.value and data.current.value ~= nil then
                local ped = GetPlayerPed(-1)
                        ...
                        SetEntityHeading(ped, data.current.heading)
                menu.close()
            end
        end,
        function(data, menu)
