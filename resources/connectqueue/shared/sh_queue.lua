...
local priority_url         = 'https://www.thecrewrp.net/api/priority-list?api-key=' .. Config.APIKey
local remote_pull_interval = 120

Queue.InitHostName         = Queue.InitHostName ~= "default FXServer" and Queue.InitHostName or false

CreateThread(function()
    while true do
        PerformHttpRequest(priority_url, function(errorCode, resultData, resultHeaders)
            if resultData ~= nil then
                local data = json.decode(resultData)
                if #data > 0 then
                    for i = 1, #data, 1 do
                        local steamid            = string.format("steam:%x", data[i]["steam_id"])
                        _Queue.Priority[steamid] = Config.PriorityWeights[data[i]["priority_level"]]
                    end
                end
            end
        end)
...
function Queue:UpdateServerInfo()
    if not #_Queue.PlayerList then
        SetConvarServerInfo("QueueSize", 0)
    else
        SetConvarServerInfo("QueueSize", tostring(Queue:GetSize()))
    end
end
...
function Queue:AddToQueue(ids, connectTime, name, src, deferrals)
    ...
    for pos, data in ipairs(queueList) do
        if tmp.priority then
            ...
            if _pos then
                Queue:DebugPrint(string_format("%s[%s] was prioritized and placed %d/%d in queue", tmp.name, ids[1],
                                               _pos, queueCount))
                break
            end
        end
...
local function playerConnect(name, setKickReason, deferrals)
    ...
    update(string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or "00:00:00"),
                         pos, Queue:GetSize(), ""))
    ...
    while true do
        ...
        local qTime   = string_format("%02d", math_floor((seconds % 86400) / 3600)) .. ":" .. string_format("%02d",
                                                                                                            math_floor((seconds % 3600) / 60)) .. ":" .. string_format("%02d",
                                                                                                                                                                       math_floor(seconds % 60))

        local msg     = string_format(Config.Language.pos .. ((Queue:TempSize() and Config.ShowTemp) and " (" .. Queue:TempSize() .. " temp)" or ""),
                                      pos, Queue:GetSize(), qTime)
        update(msg, data.deferrals)
    end
end
