...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if not Config.OnlyPolicemen or (Config.OnlyPolicemen and ESX.PlayerData["job"] and ESX.PlayerData["job"]["name"] == "police") then
            ...
            local dstCheck  = GetDistanceBetweenCoords(pedCoords, Config.Armory["x"], Config.Armory["y"],
                                                       Config.Armory["z"], true)
...
OpenWeaponStorage  = function()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), "police_armory_weapon_menu",
                     ...
                     function(data, menu)
                         ...
                         if IsEntityPlayingAnim(closestPed, "mp_cop_armoury", "pistol_on_counter_cop",
                                                3) or IsEntityPlayingAnim(closestPed, "mp_cop_armoury",
                                                                          "rifle_on_counter_cop", 3) then
                         ...
                         if DoesEntityExist(closestPed) and closestPedDst <= 5.0 then
                             ...
                             if data.current["weapon"]["components"] then
                                 for i = 1, #data.current["weapon"]["components"], 1 do
                                     GiveWeaponComponentToPed(PlayerPedId(), weaponHash,
                                                              data.current["weapon"]["components"][i])
                                 end
                             end
...
RefreshPed         = function(spawn)
    ...
    ESX.TriggerServerCallback("qalle_policearmory:pedExists", function(Exists)
        if Exists and not spawn then
            ...
            local pedId = CreatePed(5, Location["hash"], Location["x"], Location["y"], Location["z"] - 0.985,
                                    Location["h"], true)
