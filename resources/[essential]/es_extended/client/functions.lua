...
ESX.ShowNotification                 = function(msg)
	TriggerServerEvent('tcrp:generalnotif', msg)
...
ESX.Game.GetVehicleProperties        = function(vehicle)
	...
	return {
		...
		modCarLivery      = GetVehicleLivery(vehicle),
	}
end
...
ESX.Game.SetVehicleProperties        = function(vehicle, props)
	...
	if props.modCarLivery ~= nil then
		SetVehicleLivery(vehicle, props.modCarLivery)
...
ESX.ShowInventory                    = function()
	...
	for i = 1, #ESX.PlayerData.accounts, 1 do
		if ESX.PlayerData.accounts[i].money > 0 then
			...
			table.insert(elements, {
				label     = ('%s: <span style="color:green;">%s</span>'):format(ESX.PlayerData.accounts[i].label,
																				formattedMoney),
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'inventory',
			...
			if data.current.usable then
				table.insert(elements,
							 { label = _U('use'), action = 'use', type = data.current.type, value = data.current.value })
			end

			if data.current.canRemove then
				if player ~= -1 and distance <= 3.0 then
					table.insert(elements,
								 { label = _U('give'), action = 'give', type = data.current.type, value = data.current.value })
				end

				table.insert(elements,
							 { label = _U('remove'), action = 'remove', type = data.current.type, value = data.current.value })
			end

			if data.current.type == 'item_weapon' and data.current.ammo > 0 and player ~= -1 and distance <= 3.0 then
				table.insert(elements,
							 { label = _U('giveammo'), action = 'giveammo', type = data.current.type, value = data.current.value })
			end
					...
					if data1.current.action == 'give' then
						...
						ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'give_item_to',
								...
								if type == 'item_weapon' then
									...
									TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), type,
													   item, sourceAmmo)
									menu2.close()
									menu1.close()
									...
									ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
										...
										if quantity ~= nil then
											TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer),
															   type, item, quantity)
