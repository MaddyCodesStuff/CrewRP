	...
	TriggerEvent('es:getPlayerFromId', _source, function(player)
		...
		table.insert(tasks, function(cb)
			...
			MySQL.Async.fetchAll('SELECT * FROM `user_inventory` WHERE `identifier` = @identifier', {
				...
				Async.parallelLimit(tasks2, 5, function(results)
				end)
		...
		Async.parallel(tasks, function(results)
			...
			local xPlayer = CreateExtendedPlayer(player, userData.accounts, userData.inventory, userData.job,
												 userData.loadout, userData.playerName, userData.lastPosition)
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount, sCharName, tCharName)
	local _source       = source
	...
	if not sCharName then
		sCharName = sourceXPlayer.name
	end

	if not tCharName then
		tCharName = targetXPlayer.name
	end
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			...
			if targetItem.limit ~= -1 and (targetItem.count + itemCount) > targetItem.limit then
				TriggerClientEvent('esx:showNotification', _source, _U('ex_inv_lim', tCharName))
			else
				sourceXPlayer.removeInventoryItem(itemName, itemCount)
				targetXPlayer.addInventoryItem(itemName, itemCount)

				TriggerClientEvent('esx:showNotification', _source,
								   _U('gave_item', itemCount, ESX.Items[itemName].label, tCharName))
				TriggerClientEvent('esx:showNotification', target,
								   _U('received_item', itemCount, ESX.Items[itemName].label, sCharName))
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', _source,
							   _U('gave_money', ESX.Math.GroupDigits(itemCount), tCharName))
			TriggerClientEvent('esx:showNotification', target,
							   _U('received_money', ESX.Math.GroupDigits(itemCount), sCharName))
	...
	elseif type == 'item_account' then
		...
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			TriggerClientEvent('esx:showNotification', _source,
							   _U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.AccountLabels[itemName],
								  tCharName))
			TriggerClientEvent('esx:showNotification', target,
							   _U('received_account_money', ESX.Math.GroupDigits(itemCount),
								  Config.AccountLabels[itemName], sCharName))
	...
	elseif type == 'item_weapon' then
		...
		if not targetXPlayer.hasWeapon(itemName) then
			...
			if itemCount > 0 then
				TriggerClientEvent('esx:showNotification', _source,
								   _U('gave_weapon_ammo', weaponLabel, itemCount, tCharName))
				TriggerClientEvent('esx:showNotification', target,
								   _U('received_weapon_ammo', weaponLabel, itemCount, sCharName))
			else
				TriggerClientEvent('esx:showNotification', _source, _U('gave_weapon', weaponLabel, tCharName))
				TriggerClientEvent('esx:showNotification', target, _U('received_weapon', weaponLabel, sCharName))
			end
	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				local pickupLabel = ('~y~%s~s~ [~g~%s~s~]'):format(_U('cash'),
																   _U('locale_currency',
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('~y~%s~s~ [~g~%s~s~]'):format(account.label,
																   _U('locale_currency',
																	  ESX.Math.GroupDigits(itemCount)))
...
AddEventHandler('esx:onPickup', function(id)
	...
	if xPlayer then
		...
		print("Unable to find xPlayer from ID " .. _source .. " (es_extended/server/main.lua:498 -> citRa)")
	end
end)
...
TriggerEvent("es:addGroup", "jobmaster", "user", function(group)
end)
