TriggerEvent('es:addGroupCommand', 'tp', 'admin', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
			 ...
			 { help = "Teleport to coordinates", params = { { name = "x", help = "X coords" }, { name = "y", help = "Y coords" }, { name = "z", help = "Z coords" } } })
...
TriggerEvent('es:addGroupCommand', 'setjob', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then
			...
			if #result < 1 then
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Invalid job/grade combination.' } })
			else
				xPlayer.setJob(args[2], tonumber(args[3]))
				TriggerEvent('tcrp-watchdog:auditlog','LOG: **'.. sPlayer.name ..'[ID: '.. source ..'] **set job ['.. args[2] .. ' at grade '.. tonumber(args[3]) ..'] on **'.. xPlayer.name ..' [Player ID: '.. args[1] ..']**')
			end
		else
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
			 ...
			 { help = _U('setjob'), params = { { name = "id", help = _U('id_param') }, { name = "job", help = _U('setjob_param2') }, { name = "grade_id", help = _U('setjob_param3') } } })
...
TriggerEvent('es:addGroupCommand', 'setmoney', 'admin', function(source, args, user)
	...
	if target and money_type and money_amount and xPlayer ~= nil then
		...
		elseif money_type == 'black' then
			...
			TriggerClientEvent('chatMessage', _source, "SYSTEM", { 255, 0, 0 },
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
			 ...
			 { help = _U('setmoney'), params = { { name = "id", help = _U('id_param') }, { name = "money type", help = _U('money_type') }, { name = "amount", help = _U('money_amount') } } })
...
TriggerEvent('es:addGroupCommand', 'giveaccountmoney', 'admin', function(source, args, user)
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
			 ...
			 { help = _U('giveaccountmoney'), params = { { name = "id", help = _U('id_param') }, { name = "account", help = _U('account') }, { name = "amount", help = _U('money_amount') } } })
...
TriggerEvent('es:addGroupCommand', 'giveitem', 'admin', function(source, args, user)
	...
	if count ~= nil then
		if xPlayer.getInventoryItem(item) ~= nil then
			...
			TriggerEvent('tcrp-watchdog:auditlog',
				'LOG: **'.. sPlayer.name ..'[ID: '.. source ..']** gave item ['.. args[2] .. ' (x'.. tonumber(args[3]) ..')] to **'.. xPlayer.name ..' [Player ID: '.. args[1] ..']**'
			)
		else
	...
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
			 ...
			 { help = _U('giveitem'), params = { { name = "id", help = _U('id_param') }, { name = "item", help = _U('item') }, { name = "amount", help = _U('amount') } } })
...
TriggerEvent('es:addGroupCommand', 'giveweapon', 'admin', function(source, args, user)
	...
	TriggerEvent('tcrp-watchdog:auditlog',
		'LOG: **'.. sPlayer.name ..'[ID: '.. source ..']** gave weapon ['.. args[2] .. ' (x'.. tonumber(args[3]) ..')] to **'.. xPlayer.name ..' [Player ID: '.. args[1] ..']**'
	)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Insufficient Permissions.' } })
end,
			 { help = _U('giveweapon'), params = { { name = "id", help = _U('id_param') }, { name = "weapon", help = _U('weapon') }, { name = "ammo", help = _U('amountammo') } } })
