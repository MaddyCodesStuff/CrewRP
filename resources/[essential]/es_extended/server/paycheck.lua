	...
	function payCheck()
		...
		for i = 1, #xPlayers, 1 do
			...
			if salary > 0 then
						...
						if society ~= nil then
							...
							TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
								if account.money >= salary then
									...
									TriggerClientEvent('tcrp:displayBank', xPlayer.source, salary)
								else
				...
				-- 					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'),
				-- 									   _U('received_paycheck'), _U('received_salary', salary),
				-- 									   'CHAR_BANK_MAZE', 9)
				-- 				else
				-- 					TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), '',
				-- 									   _U('company_nomoney'), 'CHAR_BANK_MAZE', 1)
				...
				-- 			xPlayer.addAccountMoney('bank', salary)
				-- 			TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'),
				-- 							   _U('received_paycheck'), _U('received_salary', salary), 'CHAR_BANK_MAZE',
				-- 							   9)
				-- 		end
				-- 	end)
				-- else
